...
def bot_shutdown_keyboard(mode="none", back_callback="owner:back"):
    # mode مشخص می‌کند چه گروهی خاموش است: users=کاربران، admins=ادمین‌ها، all=هر دو.
    users_on = mode in {"none", "admins"}
    admins_on = mode in {"none", "users"}
    users_label = f"{'🟢' if users_on else '🔴'} {'روشن' if users_on else 'خاموش'} برای کاربران عادی"
    admins_label = f"{'🟢' if admins_on else '🔴'} {'روشن' if admins_on else 'خاموش'} برای ادمین‌ها"
    all_on = mode == "none"
    all_label = f"{'🟢' if all_on else '🔴'} {'روشن' if all_on else 'خاموش'} برای همه"
    return InlineKeyboardMarkup([
...
def missile_edit_keyboard(mid, back="owner:missile_list", active=False):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📦 ظرفیت اشغال کلی", callback_data=f"owner:missile_edit:{mid}:base_capacity")],
...
def missile_base_settings_keyboard(mid):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⏱️ زمان رسیدن به هدف پایه (ثانیه)", callback_data=f"owner:missile_edit:{mid}:base_target_time")],
...
def missile_level_edit_keyboard(mid, level):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⏱️ زمان رسیدن به هدف", callback_data=f"owner:missile_level_edit:{mid}:{level}:target_time")],
