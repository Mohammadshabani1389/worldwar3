...
def _zip_database() -> Path:
    ...
    tmp_db=BACKUP_DIR/f"database_{stamp}.db.tmp"
    archive=BACKUP_DIR/f"worldwar3_backup_{stamp}.zip"
    src=sqlite3.connect(str(source), timeout=30); dst=sqlite3.connect(str(tmp_db))
    try:
        with dst: src.backup(dst)
    finally:
        dst.close(); src.close()
    try:
        with zipfile.ZipFile(archive,"w",compression=zipfile.ZIP_DEFLATED,compresslevel=6) as zf: zf.write(tmp_db,arcname="database.db")
    finally:
        try: tmp_db.unlink(missing_ok=True)
        except Exception: log.exception("Could not remove temporary database snapshot")
    return archive
...
async def _send_backup(bot, archive: Path) -> bool:
    ...
    with archive.open("rb") as fh:
        await bot.send_document(chat_id=int(OWNER_ID),document=fh,filename=archive.name,caption=caption,parse_mode="HTML")
    return True

async def _retry_pending(app):
    pending=sorted(BACKUP_DIR.glob("worldwar3_backup_*.zip"))
