...
def _default_bank_config():
    ...
    for level in range(1, 101):
        levels[str(level)] = {
            ...
            "loan": {"min_amount": 0, "max_amount": 0, "min_interest_percent": 0, "max_interest_percent": 0, "min_installments": 1, "max_installments": 1, "max_active_loans": 1},
            "tax_min": 0,
            "tax_max": 0,
            "active": True,
            "investments": [],
...
def get_bank_config(session):
    ...
    for level in range(1,101):
        ...
        loan_defaults = {"min_amount": 0, "max_amount": 0, "min_interest_percent": 0, "max_interest_percent": 0, "min_installments": 1, "max_installments": 1, "max_active_loans": 1}
...
def accrue_bank_tax(session,country):
    """Accrue citizen tax continuously; one full day equals citizens * tax_per_citizen."""
    from datetime import datetime
    acc=ensure_bank_account(session,country); now=datetime.utcnow()
    last=acc.tax_last_accrual_at or now
    elapsed=max(0.0,(now-last).total_seconds())
    daily=max(0.0,float(acc.citizens or 0))*max(0.0,float(acc.tax_per_citizen or 0))
