...
def make_challenge(digits=5,n=4):
    """Create a finite set of numeric challenge options.

    The previous implementation generated distractors only by shuffling the
    answer. That can never produce a different value for answers such as
    11111, and it can loop forever when digits=1. Generate independent
    same-length numbers instead, with a bounded number of attempts.
    """
    ...
    while len(vals)<target and attempts < target*30:
        ...
        attempts += 1
    # The bounded random loop is sufficient for normal settings; this fallback
    # guarantees completion even for unusual configurations.
    if len(vals)<target:
