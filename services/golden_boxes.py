...
def challenge_image(answer):
    ...
    # The number must be the main visual element.  Do not rely on the current
    # working directory for the font: on Windows the old relative path could
    # fall back to PIL's tiny bitmap font.
    ...
    font = None
    if font_path:
        # Start deliberately large and shrink only until the complete number
        # fits inside a small safety margin.
        for font_size in range(480, 120, -4):
            try:
                candidate = ImageFont.truetype(font_path, font_size)
                box = draw.textbbox((0, 0), answer, font=candidate, stroke_width=0)
                tw, th = box[2] - box[0], box[3] - box[1]
                if tw <= width * 0.94 and th <= height * 0.84:
                    font = candidate
                    break
            except Exception:
                break
...
def schedule_times(cfg, date_value, now=None):
    ...
    base = datetime(date_value.year, date_value.month, date_value.day, tzinfo=IRAN_TZ)
    return [
