...
def _scheduled(session, day: date, cfg, now=None):
    """Persist one complete daily plan for the current schedule generation."""
    ...
    raw = json.dumps({'generation': generation, 'events': events}, ensure_ascii=False)
    if row:
        row.value = raw
    else:
...
async def golden_scheduler(app):
    while True:
        ...
        try:
            ...
            today = now_iran.date()
            # One scheduling stream per real Telegram group.  Country rows are
            # the game's group records, but duplicate chat_ids must not receive
            ...
            if cfg.get('enabled') and weights_valid(cfg) and iran_weekday(today) in cfg.get('active_days', []):
                events = _scheduled(session, today, cfg, now=now_iran)
                day_key = today.isoformat()
                generation = int(cfg.get('schedule_generation', 0) or 0)
                sent_key = f'{day_key}:{generation}'
                for country in countries:
                    ...
                    for event in pending_events:
                        ...
                        if ok:
                            _mark_sent(state, sent_key, int(event.get('index', 0)))
                            save_state(session, country.id, state)
