...
def _bank_user_main_text(session, country):
    ...
    return (f"🏦 <b>بانک مرکزی — سطح {level}</b>\n\n"
        "🏦 بانک مرکزی محل مدیریت سپرده، وام و سرمایه‌گذاری کشور شماست.\n"
        f"💰 موجودی سپرده: <b>{float(acc.deposit_principal or 0):,.0f}</b> پول\n"
        f"💹 سود سپرده قابل برداشت: <b>{float(acc.accrued_profit or 0):,.0f}</b> پول\n\n"
        "👇 یکی از بخش‌های بانک را انتخاب کنید.")
...
def clear_all_waiting_states(context, chat_id=None):
    ...
    keys = (
        ...
        "bank_loan_quote", "bank_investment_quote", "owner_economy_confirm",
    )
    for key in keys:
...
async def group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _incoming_group_text and (is_text_command(_incoming_group_text) or _incoming_group_text.startswith("/")):
        for _key in (
            "economy_pending", "balance_pending", "country_name_pending", "country_edit_waiting",
            ...
            "shield_edit_pending", "shield_auto_setting_pending", "global_loot_percent_pending", "bank_pending", "bank_loan_quote",
        ):
...
async def private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if bank_pending and bank_pending.get("action") == "tax" and (message.text or "").strip():
            ...
            total_tax=float(acc.citizens or 0)*float(acc.tax_per_citizen or 0)
            text=(f"👥 <b>جمعیت و مالیات</b>\n\n"
                  f"👥 تعداد شهروندان: <b>{int(acc.citizens or 0):,}</b>\n"
                  f"🧾 مالیات هر شهروند: <b>{float(acc.tax_per_citizen or 0):,.0f}</b>\n"
                  f"💰 مالیات یک روز کامل: <b>{total_tax:,.0f}</b>\n💰 مالیات ذخیره‌شده: <b>{float(acc.accrued_tax or 0):,.0f}</b>\n\n"
                  "از دکمه زیر می‌توانید مالیات هر شهروند را تعیین کنید.")
        ...
        if economy_pending and not context.user_data.get("enigma_pending") and (message.text or "").strip():
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_build":
                ...
                if back == "owner:bank_build_costs":
                    ...
                    text=(f"🧾 <b>تنظیمات مالیات — سطح 1</b>\n\n⬇️ حداقل مالیات هر شهروند: <b>{float(d.get('tax_min',0) or 0):,.0f}</b>\n⬆️ حداکثر مالیات هر شهروند: <b>{float(d.get('tax_max',0) or 0):,.0f}</b>")
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "population_initial":
                ...
                await _return_to_bank_panel(context,pending,f"👥 <b>جمعیت اولیه</b>\n\nمقدار فعلی: <b>{value:,}</b> شهروند",__import__("keyboards.admin",fromlist=["initial_settings_keyboard"]).initial_settings_keyboard(),message); return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_overdue_multiplier":
                ...
                text=f"💳 <b>تنظیمات قسط</b>\n\n💳 فاصله پرداخت هر قسط: <b>{get_bank_loan_interval_hours(session):g} ساعت</b>\n🔒 مدت بسته بودن بانک پس از عدم پرداخت: <b>{get_bank_loan_lock_days(session):g} روز</b>\n📈 ضریب افزایش قسط معوق: <b>×{multiplier:g}</b>\n🔔 زمان یادآوری قسط: <b>{get_bank_loan_reminder_time(session)}</b>"
                await _return_to_bank_panel(context,pending,text,installment_settings_keyboard(get_bank_loan_interval_hours(session),get_bank_loan_reminder_time(session),get_bank_loan_lock_days(session),multiplier),message); return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_lock_days":
                ...
                try:
                    ...
                    await message.reply_text("❌ مدت بسته بودن بانک باید صفر یا بیشتر باشد؛ مثلاً 2 یا 3.5 روز."); return
                set_bank_loan_lock_days(session,days); session.commit()
                pending=dict(economy_pending); context.user_data.pop("economy_pending",None); context.user_data.pop(_waiting_scope_key("economy_pending"),None)
                from keyboards.admin import installment_settings_keyboard
                text=f"💳 <b>تنظیمات قسط</b>\n\n💳 فاصله پرداخت هر قسط: <b>{get_bank_loan_interval_hours(session):g} ساعت</b>\n🔒 مدت بسته بودن بانک پس از عدم پرداخت: <b>{days:g} روز</b>\n🔔 زمان یادآوری قسط: <b>{get_bank_loan_reminder_time(session)}</b>"
                await _return_to_bank_panel(context,pending,text,installment_settings_keyboard(get_bank_loan_interval_hours(session),get_bank_loan_reminder_time(session),days),message); return

            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_reminder_time":
                raw=str(message.text or "").strip()
                try:
                    h,m=map(int,raw.split(":",1))
                    if not (0<=h<=23 and 0<=m<=59): raise ValueError
                    set_bank_loan_reminder_time(session,f"{h:02d}:{m:02d}"); session.commit()
                except Exception:
                    await message.reply_text("❌ زمان نامعتبر است. فرمت درست: ساعت:دقیقه، مثل 09:00")
                    return
                context.user_data.pop("economy_pending",None); context.user_data.pop(_waiting_scope_key("economy_pending"),None)
                back=economy_pending.get("back_callback","owner:installment_settings")
                await message.reply_text(f"✅ زمان یادآوری قسط روی <b>{h:02d}:{m:02d}</b> تنظیم شد.",parse_mode="HTML")
                try:
                    await context.bot.edit_message_text(chat_id=int(economy_pending.get("prompt_chat_id")),message_id=int(economy_pending.get("prompt_message_id")),text=f"💳 <b>تنظیمات قسط</b>\n\n💳 فاصله پرداخت هر قسط: <b>{get_bank_loan_interval_hours(session):g} ساعت</b>\n🔒 مدت بسته بودن بانک پس از عدم پرداخت: <b>{get_bank_loan_lock_days(session):g} روز</b>\n📈 ضریب افزایش قسط معوق: <b>×{get_bank_loan_overdue_multiplier(session):g}</b>\n🔔 زمان یادآوری قسط: <b>{get_bank_loan_reminder_time(session)}</b>",parse_mode="HTML",reply_markup=__import__("keyboards.admin",fromlist=["installment_settings_keyboard"]).installment_settings_keyboard(get_bank_loan_interval_hours(session), get_bank_loan_reminder_time(session), get_bank_loan_lock_days(session), get_bank_loan_overdue_multiplier(session)))
                except Exception:
                    pass
                return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_interval":
                ...
                await _return_to_bank_panel(context,pending,text,installment_settings_keyboard(hours,get_bank_loan_reminder_time(session),get_bank_loan_lock_days(session),get_bank_loan_overdue_multiplier(session)),message); return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_investment":
                ...
                if 0<=idx<len(plans):
                    ...
                    text=(f"📈 <b>{escape(str(p.get('name','سرمایه‌گذاری')))}</b>\n\n"
                          ...
                          f"📈 سود احتمالی: <b>{float(p.get('profit_percent',0) or 0):g}%</b>\n"
                          f"📉 زیان احتمالی: <b>{float(p.get('loss_percent',0) or 0):g}%</b>\n"
                    ...
                    await _return_to_bank_panel(context,pending,text,bank_investment_edit_keyboard(idx, level=level, build=(level==1 and pending.get("mode") == 1)),message)
                    return
