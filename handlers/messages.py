    ...
    with SessionLocal() as session:
        ...
        if golden_pending and _waiting_scope_matches(context, 'golden_stage_scope', message.chat_id) and (message.text or '').strip():
            ...
            try:
                ...
                if typ=='reward_level':
                    lv=max(1,min(100,int(float(norm)))); context.user_data['golden_pending']={'type':'reward_value','box':gp['box'],'level':lv,'back':f'owner:golden_box:{gp["box"]}','prompt_chat_id':int(message.chat_id)}; context.user_data[_waiting_scope_key('golden_stage_scope')]=int(message.chat_id); await message.reply_text(f'🎁 <b>مقدار جایزه سطح {lv} جعبه {BOXES[gp["box"]][1]}</b> را به صورت عددی ارسال کنید.', parse_mode='HTML'); return
                if typ=='reward_value':
                    ...
                    return
                if typ=='cost_level':
                    lv=max(2,min(100,int(float(norm)))); context.user_data['golden_pending']={'type':'cost_value','box':gp['box'],'level':lv,'back':f'owner:golden_box:{gp["box"]}','prompt_chat_id':int(message.chat_id),'prompt_message_id':int(gp.get('prompt_message_id') or 0)}; context.user_data[_waiting_scope_key('golden_stage_scope')]=int(message.chat_id)
                    await message.reply_text(f'💰 <b>هزینه ارتقای جعبه {BOXES[gp["box"]][1]} در سطح {lv}</b> را به صورت عددی ارسال کنید.', parse_mode='HTML'); return
                if typ=='cost_value':
