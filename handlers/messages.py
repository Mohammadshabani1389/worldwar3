...
async def private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if command_key(message.text or "") in {command_key("بانک مرکزی"), command_key("🏦 بانک مرکزی")}:
            ...
            session.commit()
            await message.reply_text(text, parse_mode="HTML", reply_markup=central_bank_keyboard(built=True, can_upgrade=(int(country.bank_level or 0) < bank_max_level(session)), session=session, country=country))
            await message.reply_text("🏦", reply_markup=page_back_keyboard())
            return
