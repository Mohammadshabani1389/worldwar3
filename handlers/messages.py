...
def _bank_user_main_text(session, country):
    from services.economy import accrue_bank_profit, bank_level_config, get_bank_config
...
from services.economy import get_metal_mine_config, save_metal_mine_config, get_arsenal_config, get_hq_config, get_missiles_config, save_missiles_config, get_config as get_shield_config, save_config as save_shield_config, get_shield_auto_settings, get_bank_config, save_bank_config, ensure_bank_account, bank_deposit, bank_deposit_capacity, bank_level_config, bank_max_level
...
async def private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if bank_pending and bank_pending.get("action") == "tax" and (message.text or "").strip():
            ...
            acc=ensure_bank_account(session,country); acc.tax_per_citizen=value; session.commit(); context.user_data.pop("bank_pending",None); context.user_data.pop(_waiting_scope_key("bank_pending"),None)
            await message.reply_text(f"🧾 <b>مالیات تنظیم شد</b>\n\nمالیات هر شهروند: <b>{value:g}</b>",parse_mode="HTML",reply_markup=central_bank_population_keyboard())
            return
        ...
        if bank_pending and bank_pending.get("action") == "deposit" and (message.text or "").strip():
            ...
            try:
                ...
                await message.reply_text("❌ مبلغ باید عددی باشد."); return
            country = get_default_country(session, user)
            ...
            if not ok:
                await message.reply_text("❌ " + str(result)); return
            ...
            await message.reply_text(f"🏦 <b>بانک مرکزی — سطح {country.bank_level}</b>\n\n💰 سپرده: <b>{acc.deposit_principal:,.0f}</b>\n📈 سود قابل برداشت: <b>{acc.accrued_profit:,.0f}</b>\n📦 ظرفیت سپرده: <b>{float(lv.get('deposit_capacity', 0) or 0):,.0f}</b>", parse_mode="HTML", reply_markup=central_bank_deposit_keyboard())
            return
        ...
        if bank_pending and bank_pending.get("action") == "loan_amount" and (message.text or "").strip():
            ...
            try:
                ...
                await message.reply_text("❌ مبلغ وام باید عددی باشد."); return
            country=get_default_country(session,user)
            if not country or int(bank_pending.get("country_id",0))!=int(country.id):
            ...
            if active_count>=policy.max_active_loans:
                context.user_data.pop("bank_pending",None); context.user_data.pop(_waiting_scope_key("bank_pending"),None)
                await message.reply_text("❌ به حداکثر تعداد وام فعال همزمان رسیده‌اید."); return
        ...
        if bank_pending and bank_pending.get("action") == "investment_amount" and (message.text or "").strip():
            ...
            except ValueError: await message.reply_text("❌ مبلغ سرمایه‌گذاری باید عددی باشد."); return
            country=get_default_country(session,user)
            if not country or int(country.id)!=int(bank_pending.get("country_id",0)):
                context.user_data.pop("bank_pending",None); context.user_data.pop(_waiting_scope_key("bank_pending"),None); return
            cfg=get_bank_config(session); plans=cfg.get("investments",[]) or []; idx=int(bank_pending.get("plan_index",-1))
            if idx<0 or idx>=len(plans) or not bool(plans[idx].get("active",True)):
                context.user_data.pop("bank_pending",None); context.user_data.pop(_waiting_scope_key("bank_pending"),None); await message.reply_text("❌ طرح دیگر فعال نیست."); return
            plan=plans[idx]; mn=float(plan.get("min_amount",0) or 0); mx=float(plan.get("max_amount",0) or 0)
            if amount<mn or (mx>0 and amount>mx): await message.reply_text(f"❌ مبلغ باید بین {mn:,.0f} و {mx:,.0f} باشد." if mx>0 else f"❌ مبلغ باید حداقل {mn:,.0f} باشد."); return
            if not getattr(country,"infinite_money",False) and float(country.money or 0)<amount: await message.reply_text("❌ پول کافی ندارید."); return
            risk=max(float(plan.get("loss_percent",0) or 0),float(plan.get("difficulty_percent",0) or 0))
            context.user_data["bank_investment_quote"]={
                "amount":amount,"country_id":country.id,"plan_index":idx,"risk":risk,
            ...
            await message.reply_text(f"📈 <b>پیشنهاد سرمایه‌گذاری</b>\n\nطرح: <b>{escape(str(plan.get('name','سرمایه‌گذاری')))}</b>\n💰 مبلغ: <b>{amount:,.0f}</b>\n🎚️ ریسک: <b>{risk:g}%</b>\n📈 سود احتمالی: <b>{float(plan.get('profit_percent',0) or 0):g}%</b>\n📉 زیان احتمالی: <b>{float(plan.get('loss_percent',0) or 0):g}%</b>\n⏱️ مدت: <b>{float(plan.get('duration_hours',0) or 0):g} ساعت</b>\n\n⚠️ نتیجه می‌تواند سود یا زیان داشته باشد. با تأیید، مبلغ از پول کشور کسر و سرمایه‌گذاری آغاز می‌شود.",parse_mode="HTML",reply_markup=central_bank_investment_confirm_keyboard(idx))
            return
        ...
        if economy_pending and not context.user_data.get("enigma_pending") and (message.text or "").strip():
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_build":
                ...
                await _return_to_bank_panel(context,pending,text,bank_build_keyboard(),message)
                return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_reminder_time":
                ...
                try:
                    await context.bot.edit_message_text(chat_id=int(economy_pending.get("prompt_chat_id")),message_id=int(economy_pending.get("prompt_message_id")),text=f"💳 <b>تنظیمات قسط</b>\n\n💳 فاصله پرداخت هر قسط: <b>{get_bank_loan_interval_hours(session):g} ساعت</b>\n🔔 زمان یادآوری قسط: <b>{get_bank_loan_reminder_time(session)}</b>",parse_mode="HTML",reply_markup=__import__("keyboards.admin",fromlist=["installment_settings_keyboard"]).installment_settings_keyboard(get_bank_loan_interval_hours(session), get_bank_loan_reminder_time(session)))
                except Exception:
                    pass
                return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan_interval":
                ...
                await _return_to_bank_panel(context,pending,text,installment_settings_keyboard(hours,get_bank_loan_reminder_time(session)),message); return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_loan":
                cfg=get_bank_config(session); loan=cfg.setdefault("loan", {"max_amount":0,"interest_percent":0,"term_hours":0,"installments":1}); field=economy_pending.get("field"); loan[field]=int(value) if field=="installments" else value; save_bank_config(session,cfg); session.commit()
                pending = dict(economy_pending)
                context.user_data.pop("economy_pending",None); context.user_data.pop(_waiting_scope_key("economy_pending"),None)
                text=(f"💳 <b>تنظیمات وام بانک مرکزی</b>\n\n💰 حداکثر وام: <b>{float(loan.get('max_amount',0) or 0):,.0f}</b>\n📈 سود بانک: <b>{float(loan.get('interest_percent',0) or 0):g}%</b>\n⏱️ زمان بازپرداخت: <b>{float(loan.get('term_hours',0) or 0):g} ساعت</b>\n🧾 تعداد اقساط: <b>{int(loan.get('installments',1) or 1)}</b>")
                await _return_to_bank_panel(context,pending,text,bank_loans_settings_keyboard(),message)
                return

            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "bank_investment":
                cfg=get_bank_config(session); plans=cfg.setdefault("investments",[]); idx=int(economy_pending.get("index",-1)); field=economy_pending.get("field")
                if 0<=idx<len(plans):
                    if field == "name":
                    ...
                    text=(f"📈 <b>{escape(str(p.get('name','سرمایه‌گذاری')))}</b>\n\n"
                          f"🎚️ سختی: <b>{escape(str(p.get('difficulty','آسان')))}</b>\n"
                    ...
                    await _return_to_bank_panel(context,pending,text,bank_investment_edit_keyboard(idx),message)
                    return
            ...
            if economy_pending.get("building") == "bank" and economy_pending.get("type") == "level":
                ...
                context.user_data.pop("economy_pending",None); context.user_data.pop(_waiting_scope_key("economy_pending"),None)
                from keyboards.admin import bank_level_edit_keyboard
                ...
                if prompt_message_id and prompt_chat_id:
                    try:
                        await context.bot.edit_message_text(chat_id=int(prompt_chat_id), message_id=int(prompt_message_id), text=text, parse_mode="HTML", reply_markup=bank_level_edit_keyboard(level))
                    except Exception:
                        await message.reply_text(text,parse_mode="HTML",reply_markup=bank_level_edit_keyboard(level))
                else:
                    await message.reply_text(text,parse_mode="HTML",reply_markup=bank_level_edit_keyboard(level))
                return
