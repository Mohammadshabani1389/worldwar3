...
from services.construction_teams import get_team_status, team_purchase_quote, buy_next_team, get_construction_teams_config
...
from enigma.keyboards import owner_settings_keyboard, owner_attempts_keyboard, owner_level_edit_keyboard, owner_type_edit_keyboard, owner_type_level_edit_keyboard
    ...
    with SessionLocal() as session:
        ...
        if launch_match:
            ...
            if attacker is None or target is None:
                session.commit(); reply=await message.reply_text("❌ کشور مهاجم یا کشور هدف پیدا نشد."); _schedule_delete_after(context,message,reply,delay=30); return
            ...
            if not available:
                ...
                _schedule_delete_after(context, message, missing_msg, delay=30)
                return
            level=max(available)
            ...
            if shield:
                ...
                _schedule_delete_after(context, message, blocked_msg, delay=30)
                session.commit(); return
            if not buildings:
                session.commit()
                no_building_msg = await message.reply_text("❌ شخص هدف هیچ ساختمانی ندارد.")
                _schedule_delete_after(context, message, no_building_msg, delay=30)
                return
            if power<=0:
        ...
        if golden_pending and _waiting_scope_matches(context, 'golden_stage_scope', message.chat_id) and (message.text or '').strip():
            ...
            try:
                ...
                if typ in {'weight','max','upgrade_time','daily_count','opened_delete_minutes','unanswered_delete_minutes','opened_delete_seconds','unanswered_delete_seconds','challenge_digits','options_count'}:
                    ...
                    elif typ=='upgrade_time': cfg['levels'][gp['box']]['upgrade_seconds']=max(0,int(v))
                    ...
                    return
                if typ in {'start','end'}:
                    if len(norm)!=5 or norm[2]!=':' or int(norm[:2])>23 or int(norm[3:])>59: raise ValueError('time')
                ...
                if typ=='reward_level':
                    ...
                    next_pending={'type':'reward_value','box':gp['box'],'level':lv,'back':f'owner:golden_box:{gp["box"]}','prompt_chat_id':int(gp.get('prompt_chat_id') or message.chat_id),'prompt_message_id':int(gp.get('prompt_message_id') or 0)}
                    ...
                    await _update_waiting_panel(
                        ...
                        f'🎁 <b>مقدار جایزه سطح {lv} جعبه {BOXES[gp["box"]][1]}</b> را به صورت عددی ارسال کنید.',
                        InlineKeyboardMarkup([[InlineKeyboardButton('🔙 برگشت',callback_data=f'owner:golden_box:{gp["box"]}')]]),
                        message,
                    )
                    return
                if typ=='reward_value':
                    cfg['levels'][gp['box']]['rewards'][str(gp['level'])]=max(0,float(norm)); golden_save_config(session,cfg)
