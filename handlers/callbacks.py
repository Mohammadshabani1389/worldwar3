...
def _golden_box_text(cfg, k):
    ...
    return (
        ...
        f"⏱ زمان ارتقا: <b>{int(lv.get('upgrade_seconds', 0) or 0)} ثانیه</b>\n"
        f"🎁 تعداد سطوح جایزه: <b>{len(lv.get('rewards', {}) or {})}</b>\n"
        f"💰 تعداد سطوح هزینه: <b>{len(lv.get('costs', {}) or {})}</b>"
    )
    ...
    with SessionLocal() as session:
        ...
        if data.startswith("golden:"):
            ...
            if sub=="box" and len(parts)>=3:
                ...
                lines=["🎁 <b>تأیید ارتقای جعبه</b>","",f"{icon} <b>{name}</b>",f"📌 سطح فعلی: <b>{lv}</b>",f"⬆️ سطح بعدی: <b>{nxt}</b>",f"💰 هزینه ارتقا: <b>{cost:,.0f}</b> پول",f"⏱ زمان ارتقا: <b>{int(cfg['levels'][k].get('upgrade_seconds',0))} ثانیه</b>","","آیا ارتقا را تأیید می‌کنید؟"]
            ...
            if sub=="upgrade_confirm" and len(parts)>=3:
                ...
                duration=max(0,int(cfg['levels'][k].get('upgrade_seconds',0) or 0))
                if duration>0:
            ...
            if sub=="answer" and len(parts)>=5:
                try: cid=int(parts[2])
                except: await query.answer("❌ درخواست نامعتبر است.",show_alert=True); return
                token=parts[3]; choice=parts[4]; country=get_user_country_in_continent(session,user,cid)
                if not country: await query.answer("❌ شما عضو کشوری در این گروه نیستید.",show_alert=True); return
                st=golden_state(session,cid); active=st.get('active') or {}
                if active.get('token')!=token or active.get('status')!='open': await query.answer("⏰ این جعبه دیگر فعال نیست.",show_alert=True); return
                uid=int(query.from_user.id)
                if uid in [int(x) for x in active.get('wrong',[])]: await query.answer("❌ شما قبلاً برای این جعبه پاسخ اشتباه داده‌اید.",show_alert=True); return
                if choice != active.get('answer'):
                    active.setdefault('wrong',[]).append(uid); st['active']=active; golden_save_state(session,cid,st); await query.answer("❌ پاسخ اشتباه است؛ این جعبه دیگر برای شما قابل پاسخ نیست.",show_alert=True); return
                cfg=golden_config(session); box=active.get('box'); lv=int(st['levels'].get(box,1)); amount=golden_reward_for(cfg,box,lv); golden_apply_reward(country,box,amount); active['status']='won'; active['winner_id']=uid; active['reward']=amount; st['history'].append(active); st['active']=None; golden_save_state(session,cid,st)
                try: await query.edit_message_caption(caption=f"🏆 <b>جعبه {GOLDEN_BOXES[box][1]} باز شد!</b>\n\n👤 برنده: <b>{escape(query.from_user.first_name or 'کاربر')}</b>\n🎁 جایزه: <b>{amount:,.0f}</b> {GOLDEN_BOXES[box][1]}",parse_mode="HTML",reply_markup=None)
                except Exception: pass
                delay=int(cfg.get('opened_delete_seconds',30) or 0)
                if delay>0:
                    async def _del():
                        await _asyncio.sleep(delay)
                        try: await context.bot.delete_message(chat_id=cid,message_id=query.message.message_id)
                        except Exception: pass
                    context.application.create_task(_del())
                await query.answer(f"🏆 برنده شدید! {amount:,.0f} {GOLDEN_BOXES[box][1]} دریافت کردید.",show_alert=True); return
        ...
        if action == 'golden_edit' and len(parts)>=3:
            ...
            await query.answer(); await query.edit_message_text(f"✏️ <b>{label}</b>\n\nمقدار جدید برای «{label}» را به صورت عددی ارسال کنید.",parse_mode='HTML',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🔙 برگشت',callback_data=back)]])); return
        if action == 'golden_box_edit' and len(parts)>=4:
            ...
            context.user_data['golden_pending']={'type':typ,'box':k,'back':f'owner:golden_box:{k}'}; context.user_data[_waiting_scope_key('golden_stage_scope')]=int(query.message.chat_id)
        ...
        if action == 'golden_stats':
            ...
            countries=session.scalars(select(Country)).all(); total=won=expired=0; by={k:0 for k in GOLDEN_BOXES}
            for cc in countries:
                hs=_gs(session,cc.id).get('history',[]) or []
                for h in hs:
                    total+=1; by[h.get('box')]=by.get(h.get('box'),0)+1
                    if h.get('status')=='won': won+=1
                    elif h.get('status')=='expired': expired+=1
            await query.answer(); await query.edit_message_text(f"📊 <b>آمار جعبه‌ها</b>\n\n🎁 کل جعبه‌ها: <b>{total}</b>\n🏆 برنده‌دار: <b>{won}</b>\n⌛ بدون برنده: <b>{expired}</b>\n\n💰 پول: <b>{by.get('money',0)}</b>\n⛽ سوخت: <b>{by.get('fuel',0)}</b>\n🔩 فلز: <b>{by.get('metal',0)}</b>\n☢️ اورانیوم: <b>{by.get('uranium',0)}</b>",parse_mode='HTML',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🔙 برگشت',callback_data='owner:golden')]])); return
        if action == "enigma":
        ...
        if action == "bank_edit" and len(parts)>=4:
            level=int(parts[2]); field=parts[3]; edit_mode=parts[5] if len(parts)>=6 and parts[4]=="investment" else "0"; cfg=get_bank_config(session); d=cfg["levels"].get(str(level));
