...
def _bank_user_level_text(session, country):
    ...
    return (f"🏦 <b>بانک مرکزی — سطح {level}</b>\n\n"
        f"🏦 بانک مرکزی محل مدیریت سپرده، وام و سرمایه‌گذاری کشور شماست.\n"
...
def _bank_build_upgrade_text(session, country, target, operation):
    ...
    return (f"{'🏗️' if target<=1 else '⬆️'} <b>{operation} بانک مرکزی — سطح {target}</b>\n\n"
        ...
        f"📈 <b>سرمایه‌گذاری</b>\n{inv_text}\n\n"
        f"👥 <b>مالیات</b>\n🧾 مالیات هر شهروند: <b>{tax_min:g}</b> تا <b>{tax_max:g}</b>")
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if data.startswith("bank:"):
            ...
            now=datetime.utcnow()
            if sub != "open" and acc.bank_locked_until and acc.bank_locked_until > now:
                remain=max(0.0,(acc.bank_locked_until-now).total_seconds())
                days=remain/86400.0
                await query.answer(f"🔒 بانک مرکزی موقتاً بسته است. زمان باقی‌مانده: {days:.2f} روز", show_alert=True); return
            if sub == "open":
                if acc.bank_locked_until and acc.bank_locked_until > datetime.utcnow():
                    remain=max(0.0,(acc.bank_locked_until-datetime.utcnow()).total_seconds())/86400.0
                    text=(f"🏦 <b>بانک مرکزی</b>\n\n🔒 <b>بانک موقتاً بسته است.</b>\n⏳ زمان باقی‌مانده: <b>{remain:.2f} روز</b>\n\nعلت: عدم پرداخت به‌موقع قسط.")
                    await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="main_menu")]])); return
            ...
            if sub == "withdraw_principal":
                ...
                acc=accrue_bank_profit(session,country); session.flush()
                await query.edit_message_text(f"💰 <b>سپرده بانک مرکزی</b>\n\n💰 اصل سپرده: <b>{acc.deposit_principal:,.0f}</b>\n📈 سود قابل دریافت: <b>{acc.accrued_profit:,.0f}</b>\n📦 ظرفیت کل: <b>{bank_deposit_capacity(session,country):,.0f}</b>\n\nگزینه موردنظر را انتخاب کنید.",parse_mode="HTML",reply_markup=central_bank_deposit_keyboard()); return
            ...
            if sub == "loan_pay" and len(parts)>=3:
                ...
                if datetime.utcnow() < (loan.next_payment_at or datetime.utcnow()):
                    await query.answer("⏳ سررسید قسط بعدی هنوز نرسیده است.",show_alert=True); return
                ...
                if result.get("status")=="locked":
                    session.commit(); await query.answer(f"🔒 پول کافی نیست؛ بانک برای {get_bank_loan_lock_days(session):g} روز بسته شد.",show_alert=True); return
                if result.get("status")!="paid":
                    await query.answer("❌ پرداخت قسط انجام نشد.",show_alert=True); return
                session.commit(); await query.answer(f"✅ مبلغ {float(result.get('amount',0)):,.0f} پول بابت {int(result.get('cycles',1))} قسط پرداخت شد.",show_alert=True)
                ...
                detail=(f"💳 <b>وام #{loan.id}</b>\n\n"
                        f"💰 اصل: <b>{loan.principal:,.0f}</b> | باقی‌مانده: <b>{remain:,.0f}</b>\n"
                        f"📈 سود: <b>{loan.interest_percent:.2f}%</b> | 🧾 اقساط: <b>{loan.installments_paid}/{loan.installments}</b>\n"
                        f"📅 سررسید بعدی: <b>{_display_tehran(loan.next_payment_at) if loan.next_payment_at else 'تسویه شد'}</b> | وضعیت: <b>{status}</b>")
                await query.edit_message_text(detail,parse_mode="HTML",reply_markup=central_bank_loan_detail_keyboard(loan.id) if loan.status=="active" else central_bank_my_loans_keyboard()); return
            ...
            if sub == "my_loans":
                ...
                if not loans:
                    ...
                    for i,loan in enumerate(loans,1):
                        ...
                        status=status_names.get(str(loan.status),str(loan.status))
                        lines.append(f"{i}. 💳 <b>وام #{loan.id}</b>\n💰 اصل: <b>{float(loan.principal or 0):,.0f}</b> | باقی‌مانده: <b>{remain:,.0f}</b>\n📈 سود: <b>{float(loan.interest_percent or 0):.2f}%</b> | 🧾 اقساط: <b>{int(loan.installments_paid or 0)}/{int(loan.installments or 0)}</b>\n📅 سررسید: <b>{due}</b> | وضعیت: <b>{status}</b>")
                    text="\n\n".join(lines)
                await query.answer("📋 وام‌های من",show_alert=True)
                await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_my_loans_keyboard()); return
            if sub == "investments":
            ...
            if sub == "population":
                ...
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_population_keyboard()); return
        ...
        if data.startswith("owner:"):
            ...
            if action == "bank_loan_lock_days_edit":
                ...
                context.user_data[_waiting_scope_key("economy_pending")]=int(query.message.chat_id)
                await query.answer(); await query.edit_message_text(f"🔒 <b>مدت بسته بودن بانک پس از عدم پرداخت</b>\n\nمقدار فعلی: <b>{current:g} روز</b>\n\nتعداد روز بسته بودن بانک پس از عدم پرداخت قسط را وارد کنید.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="owner:installment_settings")]])); return
            ...
            if action == "installment_settings":
                ...
                await query.edit_message_text(
                    ...
                    parse_mode="HTML", reply_markup=installment_settings_keyboard(get_bank_loan_interval_hours(session), get_bank_loan_reminder_time(session), get_bank_loan_lock_days(session), get_bank_loan_overdue_multiplier(session))
                ); return
            ...
            if action == "bank_investments" and len(parts)>=3:
                ...
                await query.answer(); await query.edit_message_text(f"📈 <b>تنظیمات سرمایه‌گذاری — سطح {level}</b>\n\nسرمایه‌گذاری موردنظر را انتخاب کنید یا مورد جدید اضافه کنید.",parse_mode="HTML",reply_markup=bank_investments_settings_keyboard(plans,level=level,build=build_mode)); return

            if action == "bank_investment_add" and len(parts)>=4:
                level=int(parts[2]); build_mode=(parts[3]=="1")
                context.user_data["economy_pending"]={
                    "building":"bank",
                    "type":"bank_investment_add_name",
                    "level":level,
                    "mode":1 if build_mode else 0,
                    "back_callback":f"owner:bank_investments:{level}:{1 if build_mode else 0}",
                    "prompt_message_id":int(query.message.message_id),
                    "prompt_chat_id":int(query.message.chat_id),
                }
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                await query.answer()
                await query.edit_message_text(
                    f"📈 <b>افزودن سرمایه‌گذاری — سطح {level}</b>\n\n"
                    "ابتدا نام سرمایه‌گذاری را ارسال کنید.\n"
                    "طرح بعد از ایجاد به‌صورت <b>غیرفعال</b> ذخیره می‌شود و فقط Owner می‌تواند آن را فعال کند.",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data=f"owner:bank_investments:{level}:{1 if build_mode else 0}")]])
                ); return
            ...
            if action == "bank_investment" and len(parts)>=4:
                ...
                p=plans[idx]; await query.answer(); await query.edit_message_text(_investment_plan_text(p),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx,level=level,build=build_mode)); return
            ...
            if action == "bank_investment_toggle" and len(parts)>=5:
                ...
                await query.answer("🟢 طرح فعال شد." if plans[idx]["active"] else "🔴 طرح غیرفعال شد.",show_alert=True); await query.edit_message_text(_investment_plan_text(plans[idx]),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx,level=level,build=build_mode)); return
            ...
            if action == "bank_investment_multi" and len(parts)>=5:
                ...
                await query.edit_message_text(_investment_plan_text(plans[idx]),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx,level=level,build=build_mode)); return
            ...
            if action == "bank_investment_edit" and len(parts)>=6:
                ...
                if field=="risk":
                    cycle={"کم":"متوسط","متوسط":"زیاد","زیاد":"کم"}; p["risk"]=cycle.get(str(p.get("risk","متوسط")),"متوسط"); save_bank_config(session,cfg); session.commit(); await query.answer("🎚️ ریسک تغییر کرد.",show_alert=True); await query.edit_message_text(_investment_plan_text(p),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx,level=level,build=build_mode)); return
                if field=="plan_type":
                ...
                context.user_data["economy_pending"]={"building":"bank","type":"bank_investment","level":level,"index":idx,"field":field,"mode":1 if build_mode else 0,"back_callback":f"owner:bank_investment:{level}:{idx}:{1 if build_mode else 0}","current_value":current,"prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text(f"✏️ <b>ویرایش {field} — سطح {level}</b>\n\nمقدار فعلی: <b>{escape(str(current))}</b>\n\nمقدار جدید را ارسال کنید.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data=f"owner:bank_investment:{level}:{idx}:{1 if build_mode else 0}")]])); return
            ...
            if action == "bank_edit" and len(parts)>=4:
                ...
                level_back = f"owner:bank_level_costs:{level}" if field.startswith(("build_cost:","upgrade_cost:")) else (f"owner:bank_deposit_settings:{level}:0" if field in {"deposit_capacity","deposit_hourly_profit_percent"} else (f"owner:bank_tax_settings:{level}:0" if field in {"tax_min","tax_max"} else f"owner:bank_level:{level}"))
