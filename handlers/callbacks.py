    ...
    with SessionLocal() as session:
        ...
        if data.startswith("golden:"):
            ...
            if sub=="answer" and len(parts)>=5:
                ...
                st=golden_state(session,cid); actives=st.get('actives') or {}; active=actives.get(token) or {}
                ...
                if choice != active.get('answer'):
                    active.setdefault('wrong',[]).append(uid); actives[token]=active; st['actives']=actives; golden_save_state(session,cid,st); await query.answer("❌ پاسخ اشتباه است؛ این جعبه دیگر برای شما قابل پاسخ نیست.",show_alert=True); return
                cfg=golden_config(session); box=active.get('box'); lv=int(st['levels'].get(box,1)); amount=golden_reward_for(cfg,box,lv); golden_apply_reward(country,box,amount); active['status']='won'; active['winner_id']=uid; active['reward']=amount; st['history'].append(active); actives.pop(token,None); st['actives']=actives; golden_save_state(session,cid,st)
