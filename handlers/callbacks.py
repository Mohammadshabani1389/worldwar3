...
def _bank_user_level_text(session, country):
    ...
    return (f"🏦 <b>بانک مرکزی — سطح {level}</b>\n\n"
            "بخش موردنظر را از دکمه‌های زیر انتخاب کنید. مشخصات هر بخش داخل همان دکمه نمایش داده می‌شود.")
    ...
    with SessionLocal() as session:
        ...
        if data.startswith("bank:"):
            ...
            if sub == "open":
                ...
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_keyboard(built=True, can_upgrade=(int(country.bank_level or 0)<bank_max_level(session)), session=session, country=country)); return
            if sub == "build_panel":
                if int(country.bank_level or 0)>0:
            ...
            if sub == "build_confirm":
                ...
                if remaining>0:
                    ...
                    await query.answer("🎉 بانک مرکزی ساخته شد.",show_alert=True); await query.edit_message_text(_bank_user_level_text(session,country),parse_mode="HTML",reply_markup=central_bank_keyboard(built=True,can_upgrade=(bank_max_level(session)>1),session=session,country=country))
                return
            ...
            if sub == "cancel_confirm":
                ...
                if old_level > 0:
                    text=_bank_user_level_text(session,country)
                    await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_keyboard(built=True,can_upgrade=(old_level<bank_max_level(session)),session=session,country=country))
                else:
