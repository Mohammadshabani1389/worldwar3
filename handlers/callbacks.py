        ...
        if data.startswith("golden:"):
            ...
            if sub=="answer" and len(parts)>=5:
                ...
                if not country or int(country.chat_id) != actual_chat_id:
                    ...
                    return
                relation=session.scalar(select(UserCountry).where(
                    UserCountry.user_id == user.id,
                    UserCountry.country_id == cid,
                ))
                # Country leaders are always valid participants even if an older database
                # was created before the UserCountry relation was inserted for the leader.
                if relation is None and int(getattr(country, 'leader_user_id', 0) or 0) != int(query.from_user.id):
                ...
                golden_apply_reward(country,box,amount)
                active['status']='won'
