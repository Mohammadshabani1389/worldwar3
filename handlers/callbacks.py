...
from keyboards.admin import (
    ...
    balance_continue_keyboard, balance_final_keyboard, balance_multiple_users_keyboard, balance_single_input_keyboard, ban_multiple_ids_keyboard, user_country_list_keyboard, user_country_delete_list_keyboard, user_country_delete_confirm_keyboard,
...
from keyboards.main import (
    ...
    country_name_confirm_keyboard, arsenal_keyboard, arsenal_missile_list_keyboard, arsenal_sell_quantity_keyboard, arsenal_sell_confirm_keyboard, country_rename_payment_keyboard, central_bank_keyboard, central_bank_deposit_keyboard, central_bank_loans_keyboard, central_bank_deposit_input_keyboard, central_bank_withdraw_confirm_keyboard, central_bank_loan_input_keyboard, central_bank_investment_input_keyboard, central_bank_loan_detail_keyboard, central_bank_investments_keyboard, central_bank_investment_plan_keyboard, shield_purchase_list_keyboard, missile_technology_keyboard, missile_technology_list_keyboard, missile_upgrade_levels_keyboard, missile_upgrade_settings_keyboard
)


def _investment_plan_text(plan, index=None):
    ptype=str(plan.get("plan_type",plan.get("type","medium")))
    type_name={"short":"کوتاه‌مدت","medium":"میان‌مدت","long":"بلندمدت"}.get(ptype,ptype)
    mn=float(plan.get("min_amount",0) or 0); mx=float(plan.get("max_amount",0) or 0)
    max_text=f"{mx:,.0f}" if mx>0 else "بدون سقف"
    prefix=f"📈 <b>{escape(str(plan.get('name','سرمایه‌گذاری')))}</b>"
    return (prefix+"\n\n"+f"🧩 نوع: <b>{type_name}</b>\n"+f"🎚️ ریسک: <b>{escape(str(plan.get('difficulty','آسان')))}</b>\n"+f"💰 مبلغ: <b>{mn:,.0f}</b> تا <b>{max_text}</b>\n"+f"📈 سود احتمالی: <b>{float(plan.get('profit_percent',0) or 0):g}%</b>\n"+f"📉 زیان احتمالی: <b>{float(plan.get('loss_percent',0) or 0):g}%</b>\n"+f"⏱️ مدت: <b>{float(plan.get('duration_hours',0) or 0):g} ساعت</b>\n"+f"🟢 وضعیت: <b>{'فعال' if bool(plan.get('active',True)) else 'غیرفعال'}</b>")


def _bank_user_level_text(session, country):
    level=max(1,int(country.bank_level or 1))
    d=bank_level_config(session,level) or {}
    acc=accrue_bank_profit(session,country)
    loan=d.get("loan",{}) or {}
    plans=[x for x in (get_bank_config(session).get("investments",[]) or []) if bool(x.get("active",True))]
    tax_min=float(d.get("tax_min",0) or 0); tax_max=float(d.get("tax_max",0) or 0)
    withdraw="فعال" if bool(d.get("withdraw_enabled",True)) else "غیرفعال"
    return (f"🏦 <b>بانک مرکزی — سطح {level}</b>\n\n"
        f"📋 <b>مشخصات سطح فعلی</b>\n"
        f"🏛️ سطح مرکز فرماندهی مورد نیاز: <b>{int(d.get('required_hq_level',1) or 1)}</b>\n"
        f"📦 ظرفیت کل سپرده: <b>{float(d.get('deposit_capacity',0) or 0):,.0f}</b> پول\n"
        f"📈 سود سپرده در ساعت: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b>\n"
        f"💵 سپرده فعلی: <b>{float(acc.deposit_principal or 0):,.0f}</b> پول\n"
        f"💹 سود قابل برداشت: <b>{float(acc.accrued_profit or 0):,.0f}</b> پول\n"
        f"↩️ برداشت: <b>{withdraw}</b>\n\n"
        f"💳 <b>وام</b>\n"
        ...
        "\n"
        f"📈 <b>سرمایه‌گذاری</b>\n🟢 طرح‌های فعال: <b>{len(plans)}</b>\n\n"
        f"👥 <b>جمعیت و مالیات</b>\n👤 جمعیت: <b>{int(acc.citizens or 0):,}</b> شهروند\n"
        f"🧾 مالیات هر شهروند: <b>{float(acc.tax_per_citizen or 0):,.0f}</b>\n"
        f"📊 محدوده مالیات: <b>{tax_min:,.0f} تا {tax_max:,.0f}</b>")
...
def _bank_build_upgrade_text(session, country, target, operation):
    ...
    plans=[x for x in (cfg.get("investments",[]) or []) if bool(x.get("active",True))]
    tax_min=float(d.get("tax_min",0) or 0); tax_max=float(d.get("tax_max",0) or 0)
    ...
    return (f"{'🏗️' if target<=1 else '⬆️'} <b>{operation} بانک مرکزی — سطح {target}</b>\n\n"
        f"📋 <b>مشخصات دقیق سطح {target}</b>\n"
        f"🏛️ سطح مرکز فرماندهی مورد نیاز: <b>{int(d.get('required_hq_level',1) or 1)}</b>\n"
        f"⏱️ زمان {('ساخت' if target<=1 else 'ارتقا')}: <b>{float(d.get(time_key,0) or 0):g} ساعت</b>\n\n"
        f"💰 <b>هزینه {('ساخت' if target<=1 else 'ارتقا')}</b>\n"
        f"💰 پول: <b>{float(cost.get('money',0) or 0):,.0f}</b>\n🔩 فلز: <b>{float(cost.get('metal',0) or 0):,.0f}</b>\n"
        f"⛽ سوخت: <b>{float(cost.get('fuel',0) or 0):,.0f}</b>\n☢️ اورانیوم: <b>{float(cost.get('uranium',0) or 0):,.2f}</b>\n\n"
        f"🏦 <b>امکانات این سطح</b>\n📦 ظرفیت سپرده: <b>{float(d.get('deposit_capacity',0) or 0):,.0f}</b> پول\n"
        f"📈 سود سپرده: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b> در ساعت\n"
        f"💳 وام: <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
        f"📈 سود وام: <b>{float(loan.get('min_interest_percent',0) or 0):g}% تا {float(loan.get('max_interest_percent',0) or 0):g}%</b>\n"
        f"📈 طرح‌های سرمایه‌گذاری فعال: <b>{len(plans)}</b>\n"
        f"👥 جمعیت: <b>{int(cfg.get('population_initial',0) or 0):,}</b> شهروند\n"
        f"🧾 مالیات هر شهروند: <b>{tax_min:,.0f} تا {tax_max:,.0f}</b>")
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if data.startswith("admin:"):
            ...
            # اگر Owner روی پنل ادمین هم کلیک کند، همان دسترسی ادمین را دارد.
            await query.answer()
        ...
        if data.startswith("bank:"):
            ...
            if sub == "open":
                ...
                if remaining > 0:
                    target = abs(int(target_build or 0))
                    td = bank_level_config(session, target) or {}
                    tc = (td.get("build_cost", {}) if target <= 1 else td.get("upgrade_cost", {})) or {}
                    operation = "ساخت" if int(country.bank_level or 0) <= 0 else "ارتقا"
                    text=(f"🏦 <b>بانک مرکزی — در حال {operation} به سطح {target}</b>\n\n"
                           f"⏳ زمان باقی‌مانده: <b>{remaining/3600:,.2f} ساعت</b>\n"
                           f"🏛️ سطح مرکز فرماندهی مورد نیاز: <b>{int(td.get('required_hq_level',1) or 1)}</b>\n"
                           f"⏱️ زمان کل {operation}: <b>{float(td.get('build_time_hours' if target <= 1 else 'upgrade_time_hours',0) or 0):g} ساعت</b>\n\n"
                           f"💰 <b>هزینه {operation}</b>\n"
                           f"💰 پول: <b>{float(tc.get('money',0) or 0):,.0f}</b>\n"
                           f"🔩 فلز: <b>{float(tc.get('metal',0) or 0):,.0f}</b>\n"
                           f"⛽ سوخت: <b>{float(tc.get('fuel',0) or 0):,.0f}</b>\n"
                           f"☢️ اورانیوم: <b>{float(tc.get('uranium',0) or 0):,.2f}</b>")
                    await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_keyboard(in_progress=True,finish_uranium_cost=bank_instant_finish_uranium_cost(country,session))); return
                if int(country.bank_level or 0) <= 0:
                    text="🏦 <b>بانک مرکزی</b>\n\nهنوز بانک مرکزی ساخته نشده است."
                    await query.answer()
                    await query.edit_message_text(text, parse_mode="HTML", reply_markup=central_bank_keyboard(built=False))
                    return
                text=_bank_user_level_text(session,country)
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_keyboard(built=True, can_upgrade=(int(country.bank_level or 0)<bank_max_level(session)))); return
            if sub == "build_panel":
                if int(country.bank_level or 0)>0:
            ...
            if sub == "build":
                ...
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏗️ تأیید ساخت بانک مرکزی",callback_data="bank:build_confirm",style="success")],[InlineKeyboardButton("❌ لغو",callback_data="bank:open",style="danger")]])); return
            ...
            if sub == "build_confirm":
                ...
                if not start_construction(country,"bank",1,float(d.get("build_time_hours",0) or 0)):
            ...
            if sub == "upgrade_confirm":
                ...
                for r in ("money","metal","fuel","uranium"):
                    amount=float(c.get(r,0) or 0)
                    if amount>0 and not getattr(country,f"infinite_{r}",False): setattr(country,r,float(getattr(country,r,0) or 0)-amount)
                if not start_construction(country,"bank",target,float(d.get("upgrade_time_hours",0) or 0)):
            ...
            if sub == "cancel":
                ...
                await query.edit_message_text(
                    ...
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("❌ بله، لغو شود", callback_data="bank:cancel_confirm", style="danger")],
            ...
            if sub == "cancel_confirm":
                ...
                if old_level > 0:
                    ...
                    d=bank_level_config(session,1); b=d.get("build_cost",{}) or {}; loan=d.get("loan",{}) or {}
                    cap=float(d.get("deposit_capacity",0) or 0); profit=float(d.get("deposit_hourly_profit_percent",0) or 0)
                    plans=cfg.get("investments",[]) or []; population=int(cfg.get("population_initial",0) or 0)
                    tax_min=float(d.get("tax_min",0) or 0); tax_max=float(d.get("tax_max",0) or 0)
                    text=(f"🏗️ <b>ساخت بانک مرکزی — سطح ۱</b>\n\n"
                          ...
                          f"📦 سپرده تا سقف <b>{cap:,.0f}</b> | 📈 سود <b>{profit:g}%</b> در ساعت\n"
                          f"💳 وام: مبلغ <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
                          f"📊 سرمایه‌گذاری: <b>{len(plans)}</b> طرح\n"
                          f"👥 جمعیت: <b>{population:,}</b>\n"
                          f"🧾 مالیات هر شهروند: <b>{tax_min:,.0f} تا {tax_max:,.0f}</b>")
            ...
            if sub == "finish_now":
                ...
                session.commit(); await query.answer(f"✅ بانک مرکزی فوراً تکمیل شد. مصرف: {float(res):,.2f} اورانیوم",show_alert=True); await query.edit_message_text(_bank_user_level_text(session,country),parse_mode="HTML",reply_markup=central_bank_keyboard(built=True,can_upgrade=(int(country.bank_level or 0)<bank_max_level(session)))); return

            if sub in {"withdraw_profit", "withdraw_all"}:
                if not bool(bank_level_config(session,max(1,int(country.bank_level or 1))).get("withdraw_enabled",True)):
                    await query.answer("❌ برداشت در این سطح بانک فعال نیست.",show_alert=True); return
                acc=accrue_bank_profit(session,country); session.flush()
                if sub == "withdraw_profit":
                    amount=float(acc.accrued_profit or 0)
                    if amount<=0:
                        await query.answer("ℹ️ در حال حاضر سود قابل برداشتی ندارید.",show_alert=True); return
                    text=f"💵 <b>تأیید برداشت سود</b>\n\nسود قابل برداشت: <b>{amount:,.0f}</b> پول\n\nآیا می‌خواهید فقط سود برداشت شود؟"
                    kind="profit"
                else:
                    amount=float(acc.deposit_principal or 0)+float(acc.accrued_profit or 0)
                    if amount<=0:
                        await query.answer("ℹ️ سپرده‌ای برای برداشت وجود ندارد.",show_alert=True); return
                    text=f"💵 <b>تأیید برداشت کل سپرده</b>\n\nاصل سپرده: <b>{float(acc.deposit_principal or 0):,.0f}</b>\nسود: <b>{float(acc.accrued_profit or 0):,.0f}</b>\n💰 مجموع دریافتی: <b>{amount:,.0f}</b> پول\n\nبا تأیید، اصل و سود هر دو برداشت می‌شوند."
                    kind="all"
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_withdraw_confirm_keyboard(kind)); return
            if sub == "withdraw_profit_confirm":
                if not bool(bank_level_config(session,max(1,int(country.bank_level or 1))).get("withdraw_enabled",True)):
                    await query.answer("❌ برداشت در این سطح بانک فعال نیست.",show_alert=True); return
                amount=bank_withdraw_profit(session,country); session.commit()
                await query.answer(f"💵 {amount:,.0f} پول برداشت شد.",show_alert=True)
                acc=accrue_bank_profit(session,country); session.flush()
                await query.edit_message_text(f"💵 <b>برداشت سود انجام شد</b>\n\n💰 اصل سپرده: <b>{acc.deposit_principal:,.0f}</b>\n📈 سود قابل برداشت: <b>{acc.accrued_profit:,.0f}</b>",parse_mode="HTML",reply_markup=central_bank_deposit_keyboard()); return
            if sub == "withdraw_all_confirm":
                if not bool(bank_level_config(session,max(1,int(country.bank_level or 1))).get("withdraw_enabled",True)):
                    await query.answer("❌ برداشت در این سطح بانک فعال نیست.",show_alert=True); return
                amount=bank_withdraw_all(session,country); session.commit()
                await query.answer(f"💵 {amount:,.0f} پول برداشت شد.",show_alert=True)
                await query.edit_message_text("💵 <b>برداشت کل سپرده انجام شد</b>\n\n💰 اصل سپرده: <b>0</b>\n📈 سود قابل برداشت: <b>0</b>",parse_mode="HTML",reply_markup=central_bank_deposit_keyboard()); return
            if sub == "deposit":
                acc=accrue_bank_profit(session,country); session.flush()
                await query.answer(); await query.edit_message_text(
                    ...
                    parse_mode="HTML", reply_markup=central_bank_deposit_keyboard()); return
            if sub == "deposit_input":
                context.user_data["bank_pending"]={"action":"deposit","country_id":country.id}; context.user_data[_waiting_scope_key("bank_pending")]=int(query.message.chat_id)
                acc=accrue_bank_profit(session,country); session.flush(); remaining=max(0,bank_deposit_capacity(session,country)-acc.deposit_principal)
            ...
            if sub == "loan_request":
                ...
                context.user_data["bank_pending"]={"action":"loan_amount","country_id":country.id}
            ...
            if sub == "loan_confirm":
                ...
                if any(abs(float(getattr(q,k))-float(pending[k])) > 0.01 for k in ("principal","interest_percent","interest_amount","total_due","installments","installment_amount","term_days","installment_interval_days")):
            ...
            if sub == "loan_pay" and len(parts)>=3:
                ...
                if not loan or int(loan.country_id)!=int(country.id) or loan.status!="active": await query.answer("❌ وام فعال پیدا نشد.",show_alert=True); return
                if datetime.utcnow() < (loan.next_payment_at or datetime.utcnow()): await query.answer(f"⏳ سررسید قسط بعدی هنوز نرسیده است.",show_alert=True); return
                amount=min(float(loan.installment_amount or 0), float(loan.total_due or 0)-float(loan.paid or 0))
                if amount<=0: loan.status="paid"; session.commit(); await query.answer("✅ وام تسویه شده است.",show_alert=True); return
                if not getattr(country,"infinite_money",False) and float(country.money or 0)<amount: await query.answer(f"❌ پول کافی نیست. مبلغ قسط: {amount:,.0f}",show_alert=True); return
                if not getattr(country,"infinite_money",False): country.money-=amount
                loan.paid=float(loan.paid or 0)+amount; loan.installments_paid=int(loan.installments_paid or 0)+1
                if loan.installments_paid>=loan.installments or loan.paid+0.01>=loan.total_due:
                    loan.paid=loan.total_due; loan.status="paid"; loan.next_payment_at=None
                else:
                    previous_due = loan.next_payment_at or datetime.utcnow()
                    loan.next_payment_at=previous_due+timedelta(hours=float(loan.installment_interval_days or 1.0)*24.0)
                    loan.last_reminder_for=None
                session.commit(); await query.answer("✅ قسط پرداخت شد.",show_alert=True)
                remain=max(0,float(loan.total_due or 0)-float(loan.paid or 0)); await query.edit_message_text(f"💳 <b>وام #{loan.id}</b>\n\n💰 اصل: <b>{loan.principal:,.0f}</b>\n📈 سود: <b>{loan.interest_percent:.2f}%</b>\n💵 کل بازپرداخت: <b>{loan.total_due:,.0f}</b>\n💵 باقی‌مانده: <b>{remain:,.0f}</b>\n🧾 اقساط پرداخت‌شده: <b>{loan.installments_paid}/{loan.installments}</b>\n📅 سررسید بعدی: <b>{_display_tehran(loan.next_payment_at) if loan.next_payment_at else 'تسویه شد'}</b>",parse_mode="HTML",reply_markup=central_bank_loan_detail_keyboard(loan.id) if loan.status=="active" else central_bank_loans_keyboard()); return

            if sub == "my_loans":
                loans=session.scalars(select(BankLoan).where(BankLoan.country_id==country.id).order_by(BankLoan.id.desc())).all()
                if not loans:
                    text="📋 <b>وام‌های من</b>\n\nهنوز هیچ وامی ثبت نشده است."
                else:
                    lines=["📋 <b>وام‌های من</b>",""]
                    for i,loan in enumerate(loans,1):
                        remain=max(0,float(loan.total_due or 0)-float(loan.paid or 0))
                        due=_display_tehran(loan.next_payment_at) if loan.next_payment_at else '—'
                        lines.append(f"{i}. 💰 اصل: <b>{float(loan.principal or 0):,.0f}</b> | باقی‌مانده: <b>{remain:,.0f}</b>\n   📈 سود: <b>{float(loan.interest_percent or 0):.2f}%</b> | 🧾 اقساط: <b>{int(loan.installments_paid or 0)}/{int(loan.installments or 0)}</b> | 📅 سررسید: <b>{due}</b> | وضعیت: <b>{escape(str(loan.status))}</b>")
                    text="\n".join(lines)
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_loans_keyboard()); return
            if sub == "investments":
            ...
            if sub == "investment_plans":
                plans=[p for p in (cfg.get("investments",[]) or []) if bool(p.get("active",True))]
                if not plans:
                    await query.answer("⚠️ در حال حاضر طرح فعالی وجود ندارد.",show_alert=True); return
                rows=[]
                for i,p in enumerate(cfg.get("investments",[]) or []):
            ...
            if sub == "investment_plan" and len(parts)>=3:
                try: idx=int(parts[2]); plan=(cfg.get("investments",[]) or [])[idx]
            ...
            if sub == "investment_amount" and len(parts)>=3:
                ...
                if not bool(plan.get("active",True)): await query.answer("❌ این طرح فعال نیست.",show_alert=True); return
                context.user_data["bank_pending"]={"action":"investment_amount","country_id":country.id,"plan_index":idx}
                context.user_data[_waiting_scope_key("bank_pending")]=int(query.message.chat_id)
                mn=float(plan.get("min_amount",0) or 0); mx=float(plan.get("max_amount",0) or 0)
            ...
            if sub == "investment_confirm" and len(parts)>=3:
                ...
                snapshot_ok=(str(plan.get("name","سرمایه‌گذاری"))==str(pending.get("name")) and str(plan.get("plan_type",plan.get("type","medium")))==str(pending.get("plan_type")) and abs(float(plan.get("profit_percent",0) or 0)-float(pending.get("profit_percent",0) or 0))<0.0001 and abs(float(plan.get("loss_percent",0) or 0)-float(pending.get("loss_percent",0) or 0))<0.0001 and abs(float(plan.get("duration_hours",0) or 0)-float(pending.get("duration_hours",0) or 0))<0.0001)
                ...
                inv=BankInvestment(country_id=country.id,plan_name=str(plan.get("name","سرمایه‌گذاری")),plan_type=str(plan.get("plan_type",plan.get("type","medium"))),principal=amount,risk=float(pending.get("risk",0) or 0),profit_percent=float(plan.get("profit_percent",0) or 0),loss_percent=float(plan.get("loss_percent",0) or 0),started_at=now,ends_at=now+timedelta(hours=duration),status="active",result=None)
            ...
            if sub == "my_investments":
                ...
                if not investments:
                    text="💼 <b>سرمایه‌گذاری‌های من</b>\n\nهنوز هیچ سرمایه‌گذاری ثبت نشده است."
                else:
                    lines=["💼 <b>سرمایه‌گذاری‌های من</b>",""]
                    for i,inv in enumerate(investments,1):
                        lines.append(f"{i}. 📈 {escape(str(inv.plan_name))} | سرمایه: <b>{float(inv.principal or 0):,.0f}</b> | وضعیت: <b>{escape(str(inv.status))}</b>")
                    text="\n".join(lines)
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=central_bank_investments_keyboard()); return
            if sub == "back":
                await query.answer()
            ...
            if sub == "population":
                ...
                from keyboards.main import central_bank_population_keyboard
                await query.answer(); await query.edit_message_text(f"👥 <b>جمعیت و مالیات</b>\n\n👥 شهروندان: <b>{acc.citizens:,}</b>\n🧾 مالیات هر شهروند: <b>{acc.tax_per_citizen:,.0f}</b>\n💰 مالیات کل بر اساس جمعیت: <b>{total_tax:,.0f}</b>\nحداقل: <b>{float(lv.get('tax_min',0) or 0):,.0f}</b>\nحداکثر: <b>{float(lv.get('tax_max',0) or 0):,.0f}</b>",parse_mode="HTML",reply_markup=central_bank_population_keyboard()); return
            await query.answer("این بخش در مرحله بعدی تکمیل می‌شود.",show_alert=True); return
        ...
        if data.startswith("owner:"):
            ...
            # هر جابه‌جایی در پنل Owner انتظار ورودی قبلی را لغو می‌کند؛
            # تا پیام بعدی اشتباهاً برای تنظیم قبلی مصرف نشود.
            if action != 'enigma_add_level':
                context.user_data.pop('enigma_pending', None)
            # ورودی‌های انیگما باید انحصاری باشند؛ اگر انتظار قدیمی دیگری
            # (اقتصاد/موجودی/ادمین و...) مانده باشد، عدد ارسالی نباید توسط آن مصرف شود.
            if action.startswith('enigma_') or action == 'enigma':
                for _k in ('economy_pending','balance_pending','admin_pending','admin_message_pending','pending_exchange','exchange_rate_pending','country_edit_waiting','country_edit','self_country_edit','missile_pending','missile_operational_pending','exchange_text_pending','missile_draft'):
            ...
            if action == "enigma":
                ...
                cfg=ensure_settings(session); await query.answer(); await query.edit_message_text("🔐 <b>مدیریت انیگما</b>",parse_mode='HTML',reply_markup=owner_main_keyboard()); return
            ...
            if action == "enigma_reset_confirm":
                ...
                cfg = ensure_settings(session)
                await query.edit_message_text("🔐 <b>مدیریت انیگما</b>", parse_mode='HTML', reply_markup=owner_main_keyboard()); return
            ...
            if action == "bank_loan_reminder_time_edit":
                current=get_bank_loan_reminder_time(session)
                context.user_data["economy_pending"]={"building":"bank","type":"bank_loan_reminder_time","back_callback":"owner:installment_settings","current_value":current,"prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text(f"💳 <b>زمان یادآوری قسط</b>\n\nزمان فعلی: <b>{current}</b>\n\nزمان جدید را به شکل ساعت:دقیقه ارسال کنید؛ مثلاً 09:00 یا 18:30.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="owner:installment_settings")]])); return
            ...
            if action == "shield_confirm_add":
                ...
                return
                # متن موفقیت در پیام قبلی فقط برای سازگاری نگه داشته شده است.
                await query.edit_message_text(f"✅ <b>سپر با موفقیت ذخیره شد.</b>\n\n🏷 نام: <b>{escape(name)}</b>\n📌 نوع: <b>{'جهانی' if typ=='global' else 'قاره‌ای'}</b>\n☢️ قیمت: <b>{price:,.0f} اورانیوم</b>\n⏱ مدت اعتبار: <b>{duration:g} ساعت</b>\n🔁 فاصله خرید مجدد: <b>{cooldown:g} ساعت</b>", parse_mode="HTML", reply_markup=shield_type_management_keyboard(typ)); return
            ...
            if action == "swap_cooldown":
                ...
                await query.answer(); await query.edit_message_text(f"⏱️ <b>محدودیت جابه‌جایی</b>\n\nمقدار فعلی: <b>{value:g} ساعت</b>\n\nعدد جدید را بر حسب ساعت ارسال کنید.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="owner:initial_settings")]]))
                context.user_data["economy_pending"]={"building":"settings","type":"swap_cooldown","back_callback":"owner:initial_settings","current_value":value}; return
                context.user_data[_waiting_scope_key("economy_pending")] = int(update.effective_chat.id)
            ...
            if action == "installment_settings":
                ...
                await query.edit_message_text(
                    ...
                    parse_mode="HTML", reply_markup=installment_settings_keyboard(get_bank_loan_interval_hours(session), get_bank_loan_reminder_time(session))
                ); return
            ...
            if action == "bank_build_edit" and len(parts)>=3:
                ...
                context.user_data["economy_pending"]={"building":"bank","type":"bank_build","field":field,"back_callback":"owner:bank_build","current_value":current,"prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                labels={"required_hq_level":"سطح مرکز فرماندهی مورد نیاز","build_time_hours":"زمان تکمیل","build_cost:money":"پول","build_cost:metal":"فلز","build_cost:fuel":"سوخت","build_cost:uranium":"اورانیوم","deposit_capacity":"حداکثر سپرده","deposit_hourly_profit_percent":"سود سپرده در ساعت (%)","tax_min":"حداقل مالیات هر شهروند","tax_max":"حداکثر مالیات هر شهروند"}
            ...
            if action == "bank_loans":
                cfg=get_bank_config(session); loan=cfg.setdefault("loan", {"max_amount":0,"interest_percent":0,"term_hours":0,"installments":1})
                await query.answer(); await query.edit_message_text(
                    f"💳 <b>تنظیمات وام بانک مرکزی</b>\n\n💰 حداکثر وام: <b>{float(loan.get('max_amount',0) or 0):,.0f}</b>\n📈 سود بانک: <b>{float(loan.get('interest_percent',0) or 0):g}%</b>\n⏱️ زمان بازپرداخت: <b>{float(loan.get('term_hours',0) or 0):g} ساعت</b>\n🧾 تعداد اقساط: <b>{int(loan.get('installments',1) or 1)}</b>",parse_mode="HTML",reply_markup=bank_loans_settings_keyboard()); return

            if action == "bank_loan_edit" and len(parts)>=3:
                field=parts[2]; cfg=get_bank_config(session); loan=cfg.setdefault("loan", {"max_amount":0,"interest_percent":0,"term_hours":0,"installments":1}); current=loan.get(field,0)
                context.user_data["economy_pending"]={"building":"bank","type":"bank_loan","field":field,"back_callback":"owner:bank_loans","current_value":current,"prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                labels={"max_amount":"حداکثر مبلغ وام","interest_percent":"سود وام (%)","term_hours":"زمان بازپرداخت (ساعت)","installments":"تعداد اقساط"}
                await query.answer(); await query.edit_message_text(f"💳 <b>{labels.get(field,field)}</b>\n\nمقدار فعلی: <b>{float(current or 0):,.2f}</b>\n\nمقدار جدید را ارسال کنید.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="owner:bank_loans")]])); return

            if action == "bank_withdraw_toggle" and len(parts)>=4:
                level=int(parts[2]); cfg=get_bank_config(session); d=cfg.get("levels",{}).get(str(level))
                if not isinstance(d,dict): await query.answer("❌ سطح بانک پیدا نشد.",show_alert=True); return
                d["withdraw_enabled"]=not bool(d.get("withdraw_enabled",True)); save_bank_config(session,cfg); session.commit()
                await query.answer("🟢 برداشت فعال شد." if d["withdraw_enabled"] else "🔴 برداشت غیرفعال شد.",show_alert=True)
                build_mode=parts[3]=="1"
                await query.edit_message_text(f"🏦 <b>تنظیمات سپرده — سطح {level}</b>\n\n📦 حداکثر سپرده: <b>{float(d.get('deposit_capacity',0) or 0):,.0f}</b>\n📈 سود ساعتی: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b>\n🔄 برداشت: <b>{'فعال' if d['withdraw_enabled'] else 'غیرفعال'}</b>",parse_mode="HTML",reply_markup=bank_deposit_settings_keyboard(level,build=build_mode)); return

            if action == "bank_investments":
                cfg=get_bank_config(session); plans=cfg.setdefault("investments",[])
                await query.answer(); await query.edit_message_text("📈 <b>تنظیمات سرمایه‌گذاری بانک مرکزی</b>\n\nسرمایه‌گذاری موردنظر را انتخاب کنید یا مورد جدید اضافه کنید.",parse_mode="HTML",reply_markup=bank_investments_settings_keyboard(plans)); return

            if action == "bank_investment_add":
                cfg=get_bank_config(session); plans=cfg.setdefault("investments",[]); plans.append({"name":"سرمایه‌گذاری جدید","plan_type":"medium","min_amount":0,"max_amount":0,"difficulty":"آسان","profit_percent":0,"loss_percent":0,"duration_hours":0,"active":True})
                save_bank_config(session,cfg); session.commit(); idx=len(plans)-1
                await query.answer("✅ سرمایه‌گذاری اضافه شد.",show_alert=True); await query.edit_message_text(f"📈 <b>سرمایه‌گذاری جدید</b>\n\nنام و ویژگی‌های آن را تنظیم کنید.",parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx)); return

            if action == "bank_investment" and len(parts)>=3:
                idx=int(parts[2]); cfg=get_bank_config(session); plans=cfg.setdefault("investments",[])
                if idx<0 or idx>=len(plans): await query.answer("❌ سرمایه‌گذاری پیدا نشد.",show_alert=True); return
                p=plans[idx]; await query.answer(); await query.edit_message_text(_investment_plan_text(p),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx)); return

            if action == "bank_investment_toggle" and len(parts)>=3:
                idx=int(parts[2]); cfg=get_bank_config(session); plans=cfg.setdefault("investments",[])
                if idx<0 or idx>=len(plans): await query.answer("❌ سرمایه‌گذاری پیدا نشد.",show_alert=True); return
                plans[idx]["active"]=not bool(plans[idx].get("active",True)); save_bank_config(session,cfg); session.commit()
                await query.answer("🟢 طرح فعال شد." if plans[idx]["active"] else "🔴 طرح غیرفعال شد.",show_alert=True)
                await query.edit_message_text(_investment_plan_text(plans[idx]),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx)); return

            if action == "bank_investment_delete" and len(parts)>=3:
                idx=int(parts[2]); cfg=get_bank_config(session); plans=cfg.setdefault("investments",[])
                if idx<0 or idx>=len(plans): await query.answer("❌ سرمایه‌گذاری پیدا نشد.",show_alert=True); return
                plans.pop(idx); save_bank_config(session,cfg); session.commit(); await query.answer("🗑️ طرح حذف شد.",show_alert=True); await query.edit_message_text("📈 <b>تنظیمات سرمایه‌گذاری بانک مرکزی</b>",parse_mode="HTML",reply_markup=bank_investments_settings_keyboard(plans)); return

            if action == "bank_investment_edit" and len(parts)>=4:
                idx=int(parts[2]); field=parts[3]; cfg=get_bank_config(session); plans=cfg.setdefault("investments",[])
                if idx<0 or idx>=len(plans): await query.answer("❌ سرمایه‌گذاری پیدا نشد.",show_alert=True); return
                p=plans[idx]; current=p.get(field,"")
                if field=="difficulty":
                    p["difficulty"]="سخت" if str(p.get("difficulty","آسان"))=="آسان" else "آسان"; save_bank_config(session,cfg); session.commit(); await query.answer("🎚️ درجه سختی تغییر کرد.",show_alert=True); await query.edit_message_text(_investment_plan_text(p),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx)); return
                if field=="plan_type":
                    order=["short","medium","long"]; cur=str(p.get("plan_type","medium")); p["plan_type"]=order[(order.index(cur)+1)%len(order)] if cur in order else "medium"; save_bank_config(session,cfg); session.commit(); await query.answer("🧩 نوع طرح تغییر کرد.",show_alert=True); await query.edit_message_text(_investment_plan_text(p),parse_mode="HTML",reply_markup=bank_investment_edit_keyboard(idx)); return
                context.user_data["economy_pending"]={"building":"bank","type":"bank_investment","index":idx,"field":field,"back_callback":f"owner:bank_investment:{idx}","current_value":current,"prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text(f"✏️ <b>مقدار جدید {field}</b>\n\nمقدار فعلی: <b>{escape(str(current))}</b>\n\nمقدار جدید را ارسال کنید.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data=f"owner:bank_investment:{idx}")]])); return
            ...
            if action == "bank_build_loan_settings":
                ...
                text=(f"💳 <b>بخش تنظیمات وام — سطح ۱ (ساخت)</b>\n\n"
                      f"💰 مبلغ: <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
                      f"📈 سود: <b>{float(loan.get('min_interest_percent',0) or 0):g}% تا {float(loan.get('max_interest_percent',0) or 0):g}%</b>\n"
            ...
            if action == "bank_loan_settings" and len(parts)>=3:
                level=int(parts[2]); d=bank_level_config(session,level); loan=d.setdefault("loan",{})
                text=(f"💳 <b>بخش تنظیمات وام — سطح {level}</b>\n\n"
                      f"💰 مبلغ: <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
                      f"📈 سود: <b>{float(loan.get('min_interest_percent',0) or 0):g}% تا {float(loan.get('max_interest_percent',0) or 0):g}%</b>\n"
                      f"🧾 اقساط: <b>{int(loan.get('min_installments',1) or 1)} تا {int(loan.get('max_installments',1) or 1)}</b>\n"
                      
                      f"👥 حداکثر وام فعال همزمان: <b>{int(loan.get('max_active_loans',1) or 1)}</b>")
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=bank_loans_settings_keyboard(level, loan=loan)); return
            ...
            if action == "bank_build_loan_edit" and len(parts)>=3:
                ...
                context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                labels={"min_amount":"حداقل مبلغ وام","max_amount":"حداکثر مبلغ وام","min_interest_percent":"حداقل سود وام (%)","max_interest_percent":"حداکثر سود وام (%)","min_installments":"حداقل تعداد اقساط","max_installments":"حداکثر تعداد اقساط","max_active_loans":"حداکثر وام فعال همزمان"}
            ...
            if action == "bank_tax_settings" and len(parts)>=3:
                ...
                text=(f"🧾 <b>تنظیمات مالیات — سطح {level}</b>\n\n"
                      f"⬇️ حداقل مالیات هر شهروند: <b>{float(d.get('tax_min',0) or 0):,.0f}</b>\n"
                      f"⬆️ حداکثر مالیات هر شهروند: <b>{float(d.get('tax_max',0) or 0):,.0f}</b>\n\n"
                      "مقدار مالیات انتخابی کاربر باید داخل این محدوده باشد.")
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=bank_tax_settings_keyboard(level)); return
            ...
            if action == "bank_deposit_settings" and len(parts)>=3:
                ...
                text=(f"🏦 <b>تنظیمات سپرده — سطح {level}</b>\n\n"
                      ...
                      f"📈 سود سپرده در ساعت: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b>\n\n"
                      "حداکثر سپرده، ظرفیت کل مخزن سپرده است.")
                await query.answer(); await query.edit_message_text(text,parse_mode="HTML",reply_markup=bank_deposit_settings_keyboard(level)); return
            ...
            if action == "bank_level_edit" and len(parts)>=3:
                ...
                text=(f"⬆️ <b>بانک مرکزی — سطح {level}</b>\n\n"
                      f"📋 <b>مشخصات دقیق سطح {level}</b>\n"
                      f"🏛️ سطح مرکز فرماندهی مورد نیاز: <b>{int(d.get('required_hq_level',1) or 1)}</b>\n"
                      f"⏱️ زمان ارتقا: <b>{float(d.get('upgrade_time_hours',0) or 0):g} ساعت</b>\n"
                      f"📦 ظرفیت سپرده: <b>{float(d.get('deposit_capacity',0) or 0):,.0f}</b>\n"
                      f"📈 سود سپرده: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b> در ساعت\n"
                      f"💳 مبلغ وام: <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
                      f"📈 سود وام: <b>{float(loan.get('min_interest_percent',0) or 0):g}% تا {float(loan.get('max_interest_percent',0) or 0):g}%</b>\n"
                      f"🧾 اقساط: <b>{int(loan.get('min_installments',1) or 1)} تا {int(loan.get('max_installments',1) or 1)}</b>\n"
                      f"👥 حداکثر وام فعال: <b>{int(loan.get('max_active_loans',1) or 1)}</b>\n"
                      f"🧾 مالیات هر شهروند: <b>{float(d.get('tax_min',0) or 0):,.0f} تا {float(d.get('tax_max',0) or 0):,.0f}</b>")
            ...
            if action == "bank_edit" and len(parts)>=4:
                level=int(parts[2]); field=":".join(parts[3:]); cfg=get_bank_config(session); d=cfg["levels"].get(str(level));
                if not d: await query.answer("❌ سطح نامعتبر است.",show_alert=True); return
                if field.startswith("upgrade_cost:"):
                    group,key=field.split(":",1); current=float(d.setdefault(group,{}).get(key,0) or 0)
                elif field.startswith("build_cost:"):
                    group,key=field.split(":",1); current=float(d.setdefault(group,{}).get(key,0) or 0)
                else:
                    group,key=None,field; current=float(d.get(key,0) or 0)
                context.user_data["economy_pending"]={"building":"bank","type":"level","level":level,"field":field,"current_value":current,"back_callback":f"owner:bank_level:{level}","prompt_message_id":int(query.message.message_id),"prompt_chat_id":int(query.message.chat_id)}; context.user_data[_waiting_scope_key("economy_pending")]=int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text(f"✏️ <b>ویرایش سطح {level}</b>\n\nمقدار فعلی: <b>{current:,.2f}</b>\n\nمقدار جدید را ارسال کنید.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data=f"owner:bank_level:{level}")]])); return
            ...
            if action == "bank_level" and len(parts)>=3:
                ...
                text=(f"{'🏗️' if level<=1 else '⬆️'} <b>بانک مرکزی — سطح {level}</b>\n\n"
                      ...
                      f"💰 هزینه {op}: <b>{float(costs.get('money',0) or 0):,.0f}</b> پول | <b>{float(costs.get('metal',0) or 0):,.0f}</b> فلز | <b>{float(costs.get('fuel',0) or 0):,.0f}</b> سوخت | <b>{float(costs.get('uranium',0) or 0):,.2f}</b> اورانیوم\n"
                      f"📦 ظرفیت سپرده: <b>{float(d.get('deposit_capacity',0) or 0):,.0f}</b>\n"
                      f"📈 سود سپرده: <b>{float(d.get('deposit_hourly_profit_percent',0) or 0):g}%</b> در ساعت\n"
                      f"💳 مبلغ وام: <b>{float(loan.get('min_amount',0) or 0):,.0f} تا {float(loan.get('max_amount',0) or 0):,.0f}</b>\n"
                      f"📈 سود وام: <b>{float(loan.get('min_interest_percent',0) or 0):g}% تا {float(loan.get('max_interest_percent',0) or 0):g}%</b>\n"
...
[InlineKeyboardButton("🔙 برگشت",callback_data="owner:economy_hq")]]
            ...
            if action == "exchange_rate" and len(parts)>=3:
                ...
                await query.answer()
                rate_text = exchange_rate_text(session)
                await query.edit_message_text("💱 <b>تنظیمات تبدیل</b>\n\n" + rate_text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="owner:economy")]]))
                return
            ...
            if action == "back_main":
                ...
                context.user_data.pop("admin_pending", None)
                session.commit()
                await query.edit_message_text("🔙 از پنل مالک خارج شدید.", reply_markup=private_main_keyboard())
                return
            ...
            if action == "list_admins":
                ...
                for i, adm in enumerate(admins, 1):
                    u = get_admin_user(session, adm.telegram_id)
                    name = (u.first_name if u and u.first_name else (u.username if u and u.username else "ثبت‌نشده"))
                    phone_line = f"\n📱 {u.phone_number}" if u and u.phone_number else ""
                    lines.append(f"{i}. 👤 {name}\n🆔 <code>{adm.telegram_id}</code>{phone_line}")
                session.commit()
                await query.edit_message_text(
                    "\n".join(lines),
                    parse_mode="HTML",
            ...
            if action in {"add_admin", "remove_admin", "search_admin"}:
                if action in {"remove_admin", "search_admin"} and not get_admins(session):
                    session.commit()
                    await query.answer("⚠️ هیچ ادمینی وجود ندارد.", show_alert=True)
                    return
                context.user_data["admin_pending"] = {"action": action}
                context.user_data[_waiting_scope_key("admin_pending")] = int(update.effective_chat.id)
                context.user_data["admin_panel"] = "owner_admins"
                context.user_data[_waiting_scope_key("admin_panel")] = int(update.effective_chat.id)
                prompts = {
                    "add_admin": "➕ <b>افزودن ادمین</b>\n\nآیدی عددی، نام کاربری، شماره تلفن یا نام کشور را ارسال کنید.",
                    "remove_admin": "➖ <b>حذف ادمین</b>\n\nآیدی عددی، نام کاربری، شماره تلفن یا نام کشور را ارسال کنید.",
                    "search_admin": "🔎 <b>جستجوی ادمین</b>\n\nآیدی عددی، نام کاربری، شماره تلفن یا نام کشور ادمین را ارسال کنید.",
                }
                session.commit()
                alert_texts = {
                    "add_admin": "➕ حالت افزودن ادمین فعال شد.",
                    "remove_admin": "➖ حالت حذف ادمین فعال شد.",
                    "search_admin": "🔎 حالت جستجوی ادمین فعال شد.",
                }
                await query.answer(alert_texts[action], show_alert=True)
                await query.edit_message_text(
                    prompts[action],
                    parse_mode="HTML",
                    reply_markup=admin_back_keyboard(),
            ...
            if action.startswith("confirm:"):
                confirm_action = action.split(":", 1)[1]
                pending = context.user_data.get("admin_pending") or {}
                target_id = pending.get("telegram_id")
                if not target_id or pending.get("action") != confirm_action:
                    await query.answer("⚠️ عملیات منقضی شده است.", show_alert=True)
                    context.user_data.pop("admin_pending", None)
                    session.commit()
                    await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                    return
                ...
                if confirm_action == "add_admin":
                    if is_owner(int(target_id), OWNER_ID):
                        ...
                        await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                        return
                    target_user = get_admin_user(session, int(target_id))
                    if target_user is None:
                        context.user_data.pop("admin_pending", None)
                        session.rollback()
                        await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                        return
                    if not add_admin(session, int(target_id)):
                        context.user_data.pop("admin_pending", None)
                        session.commit()
                        await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                        return
                    context.user_data.pop("admin_pending", None)
                    session.commit()
                    await grant_admin_command_scope(query.get_bot(), int(target_id))
                    await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                    return
                ...
                if confirm_action == "remove_admin":
                    ...
                    if result == "NOT_ADMIN":
                        ...
                        return
                    session.commit()
                    await revoke_admin_command_scope(query.get_bot(), int(target_id))
                    await query.edit_message_text("👨‍💼 <b>مدیریت ادمین‌ها</b>", parse_mode="HTML", reply_markup=admin_management_keyboard())
                    return
        ...
        if data == "arsenal_missiles":
            ...
            cfg=get_missiles_config(session)
            inv = {}
            # تا وقتی زرادخانه ساخته نشده، هیچ موشکی نباید در انبار کشور وجود داشته باشد.
            if int(getattr(country, "arsenal_level", 0) or 0) <= 0:
                inv = {}
                if get_missile_inventory(session, country.id):
                ...
                # اگر موجودی واقعی خالی/نامعتبر باشد، ظرفیت قدیمی را پاک کن تا خرید قفل نشود.
                known_ids={str(x.get("id")) for x in (cfg.get("items",[]) or []) if x.get("id") is not None}
                cleaned={mid:lv for mid,lv in inv.items() if str(mid) in known_ids and any(int(v or 0)>0 for v in (lv or {}).values())}
                if cleaned != inv:
                    save_missile_inventory(session, country.id, cleaned)
                inv=cleaned
                if not inv:
                    country.missiles=0; country.arsenal_storage=0.0; session.commit()
            items=[]
            for mid, levels in inv.items():
                ...
                for level,count in sorted((levels or {}).items(), key=lambda x:int(x[0])):
                    ...
                    if count<=0: continue
                    items.append({"mid":str(mid),"level":int(level),"label":f"{m.get('name','موشک')} — سطح {int(level)} ({count} فروند)"})
            if not items:
                # فقط Alert باز شود و پیام زرادخانه اصلاً ویرایش نشود.
                await query.answer("🚀 در حال حاضر موشکی در زرادخانه ندارید.", show_alert=True)
                return
            await query.answer()
            await query.edit_message_text("🚀 <b>لیست موشک‌های زرادخانه</b>\n\nموشک موردنظر را برای فروش انتخاب کنید:", parse_mode="HTML", reply_markup=arsenal_missile_list_keyboard(items))
            return
        ...
        if data.startswith("arsenal_build"):
            ...
            ok,res=build_arsenal(country,cfg,payment)
            if ok:
                session.commit();
                status = construction_label(country, "arsenal")
                if status:
            ...
            if res=="ALREADY_BUILT": await query.answer("⚠️ زرادخانه قبلاً ساخته شده است.",show_alert=True)
        ...
        if data.startswith("arsenal_upgrade"):
            ...
            if country is None: await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.",show_alert=True); return
            payment=data.split(":",1)[1] if ":" in data else "normal"
            cfg=get_arsenal_config(session); ok,res=upgrade_arsenal(country,cfg,payment)
            if ok:
                session.commit();
                status = construction_label(country, "arsenal")
                if status:
            ...
            if res=="MAX_LEVEL": await query.answer("🏆 به حداکثر سطح رسیدید.",show_alert=True)
        ...
        if data == "arsenal_finish_now":
            ...
            ok,res=finish_arsenal_construction_now(country,cfg)
            if not ok:
            ...
            level=int(country.arsenal_level)
            nxt=cfg.get("levels",{}).get(level+1,{})
            await query.edit_message_text(arsenal_info(country,cfg),parse_mode="HTML",reply_markup=arsenal_keyboard(False,can_upgrade_arsenal(country,cfg)[0],False,float(nxt.get("uranium",0))>0))
            return
        ...
        if data == "arsenal_cancel_confirm":
            ...
            if country is None:
                await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.",show_alert=True); return
            ok,refund=cancel_arsenal_construction(country,get_arsenal_config(session))
            if not ok:
                await query.answer("⚠️ زرادخانه در حال ساخت یا ارتقا نیست.",show_alert=True); return
            session.commit()
        ...
        if data == "arsenal_missiles":
            ...
            await query.answer()
            if not items:
                await query.edit_message_text("🚀 <b>لیست موشک‌ها</b>\n\nℹ️ در حال حاضر موشکی در زرادخانه ندارید.", parse_mode="HTML", reply_markup=arsenal_keyboard(False, can_upgrade_arsenal(country,get_arsenal_config(session))[0]))
                return
            await query.edit_message_text("🚀 <b>لیست موشک‌های زرادخانه</b>\n\nموشک موردنظر را برای فروش انتخاب کنید:", parse_mode="HTML", reply_markup=arsenal_missile_list_keyboard(items))
            return
        ...
        if data == "arsenal_refresh":
            country=get_default_country(session,user)
            if country is None:
                await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.",show_alert=True); return
            _before_level=int(getattr(country,"arsenal_level",0))
            finalize_construction(country); session.commit(); cfg=get_arsenal_config(session)
            level=int(getattr(country,"arsenal_level",0)); nxt=cfg.get("levels",{}).get(level+1,{})
            if level > _before_level:
                await query.answer(f"🎉 ارتقای زرادخانه به سطح {level} تکمیل شد.", show_alert=True)
                await query.edit_message_text(arsenal_info(country,cfg), chat_id=int(query.message.chat_id), parse_mode="HTML", reply_markup=arsenal_keyboard(False,can_upgrade_arsenal(country,cfg)[0],False,float(cfg.get("levels",{}).get(level+1,{}).get("uranium",0))>0,construction_text=construction_label(country,"arsenal"),finish_uranium_cost=arsenal_instant_finish_uranium_cost(country,cfg)))
                return
            status=construction_label(country,"arsenal")
            await query.answer("🔄 زرادخانه به‌روزرسانی شد.")
            await query.edit_message_text(arsenal_info(country,cfg),parse_mode="HTML",reply_markup=arsenal_keyboard(level==0,can_upgrade_arsenal(country,cfg)[0],float(cfg.get("build_cost",{}).get("uranium",0))>0,float(nxt.get("uranium",0))>0,construction_text=status,finish_uranium_cost=arsenal_instant_finish_uranium_cost(country,cfg)))
            return
        ...
        if data.startswith("metal_mine_build"):
            ...
            if current_hq < required_hq:
                await query.answer(f"❌ سطح مرکز فرماندهی کافی نیست.\n🏛️ سطح موردنیاز: {required_hq}", show_alert=True); return
            success, result = build_metal_mine(country, cfg, payment=payment)
            if success:
                session.commit()
        ...
        if data == "metal_mine_finish_now":
            ...
            if not resource_available(country, "uranium", cost):
                await query.answer(f"❌ اورانیوم کافی نیست.\n☢️ موردنیاز: {cost:,.2f}", show_alert=True); return
            ok, res = finish_metal_mine_construction_now(country, cfg)
            if not ok:
        ...
        if data == "metal_mine_cancel_confirm":
            ...
            if country is None:
                await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.", show_alert=True); return
            ok, refund = cancel_metal_mine_construction(country, get_metal_mine_config(session))
            if not ok:
                await query.answer("⚠️ معدن فلز در حال ساخت یا ارتقا نیست.", show_alert=True); return
            session.commit()
            await query.answer("❌ عملیات لغو شد و نصف منابع پرداختی برگشت داده شد.", show_alert=True)
        ...
        if data == "metal_mine_refresh":
            ...
            if country is None:
                await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.", show_alert=True); return
            cfg = get_metal_mine_config(session)
            _before_level=int(getattr(country,"metal_mine_level",0))
            finalize_construction(country)
            session.commit()
            if int(getattr(country,"metal_mine_level",0)) > _before_level:
                await query.answer(f"🎉 ارتقای معدن فلز به سطح {int(country.metal_mine_level)} تکمیل شد.", show_alert=True)
                try: await query.message.delete()
                except Exception: pass
                _nxt=cfg.get("levels",{}).get(int(country.metal_mine_level)+1,{})
                await context.bot.send_message(metal_mine_info(country,cfg), chat_id=int(query.message.chat_id), parse_mode="HTML", reply_markup=metal_mine_keyboard(False,True,can_upgrade_metal_mine(country,cfg)[0],False,float(_nxt.get("uranium",0))>0,construction_label(country,"metal_mine"),metal_mine_instant_finish_uranium_cost(country,cfg)))
                return
            can_upgrade = can_upgrade_metal_mine(country, cfg)[0]
            next_data = cfg.get("levels", {}).get(country.metal_mine_level + 1, {})
            can_upgrade_uranium = float(next_data.get("uranium", 0)) > 0
            status = construction_label(country, "metal_mine")
            await query.answer("🔄 به‌روزرسانی شد.")
            await query.edit_message_text(metal_mine_info(country, cfg), parse_mode="HTML", reply_markup=metal_mine_keyboard(False, True, can_upgrade, False, can_upgrade_uranium, construction_text=status, finish_uranium_cost=metal_mine_instant_finish_uranium_cost(country, cfg)))
            return
        ...
        if data == "metal_mine_collect":
            ...
            next_data = cfg.get("levels", {}).get(country.metal_mine_level + 1, {})
            max_level = int(cfg.get("max_level", 100))
            can_upgrade = country.metal_mine_level < max_level and bool(next_data) and bool(next_data.get("active", True))
            can_upgrade_uranium = float(next_data.get("uranium", 0)) > 0
            session.commit()
            amount_text = f"{amount:,.0f}"
            if amount <= 0:
                await query.answer("📦 فعلاً فلز آماده دریافت نیست.", show_alert=True)
            else:
                await query.answer(f"📥 {amount_text} فلز دریافت کردید.", show_alert=True)
            # پس از Alert، همان پیام معدن نیز با مقدار جدید مخزن رفرش می‌شود.
            await query.edit_message_text(
        ...
        if data.startswith("metal_mine_upgrade"):
            ...
            payment = data.split(":", 1)[1] if ":" in data else "normal"
            success, result = upgrade_metal_mine(country, get_metal_mine_config(session), payment=payment)
            if success:
                session.commit()
                status = construction_label(country, "metal_mine")
        ...
        if data == "hq_technology":
            ...
            await query.edit_message_text(
                ...
                parse_mode="HTML",
                reply_markup=missile_technology_keyboard(),
            )
            return
        ...
        if data.startswith("hq_tech_cancel:"):
            ...
            await query.edit_message_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ تأیید لغو",callback_data=f"hq_tech_cancel_confirm:{mid}")],
        ...
        if data == "hq_finish_now":
            ...
            if not resource_available(country,"uranium",cost):
                await query.answer(f"❌ اورانیوم کافی نیست.\n☢️ موردنیاز: {cost:,.2f}",show_alert=True); return
            ok,res=finish_hq_construction_now(country,cfg)
            if not ok:
                await query.answer("❌ تکمیل فوری انجام نشد.",show_alert=True); return
            session.commit()
        ...
        if data == "hq_cancel_confirm":
            ...
            if country is None:
                await query.answer("❌ کشور پیش‌فرضی انتخاب نشده است.", show_alert=True); return
            ok, refund = cancel_hq_construction(country, get_hq_config(session))
            if not ok:
                await query.answer("⚠️ مرکز فرماندهی در حال ساخت یا ارتقا نیست.", show_alert=True); return
            session.commit()
            await query.answer("❌ عملیات لغو شد و نصف منابع پرداختی برگشت داده شد.", show_alert=True)
        ...
        if data in {"hq_build", "hq_build:normal", "hq_build:uranium"}:
            ...
            finalize_construction(country)
            payment = data.split(":",1)[1] if ":" in data else "normal"
            success, result = build_hq(country, cfg, payment=payment)
            if success:
                session.commit()
                status = construction_label(country, "command_center")
                if status:
        ...
        if data in {"hq_upgrade", "hq_upgrade:normal", "hq_upgrade:uranium"}:
            ...
            cfg = get_hq_config(session)
            payment = data.split(":",1)[1] if ":" in data else "normal"
            success, result = upgrade_hq(country, cfg, payment=payment)
