...
from database.models import User, Admin, Country, UserCountry, BotSetting
...
from services.economy import get_metal_mine_config, save_metal_mine_config, get_arsenal_config, save_arsenal_config, get_hq_config, save_hq_config, get_missiles_config, save_missiles_config, get_initial_resources, save_initial_resources
...
from services.game import (
    ...
    calculate_exchange,
    execute_exchange,
    resource_available, construction_remaining, finalize_construction, construction_label, cancel_hq_construction,
...
from keyboards.main import (
    ...
    country_name_confirm_keyboard, arsenal_keyboard, arsenal_missile_list_keyboard, arsenal_sell_quantity_keyboard, arsenal_sell_confirm_keyboard, country_rename_payment_keyboard, missile_technology_keyboard, missile_technology_list_keyboard, missile_upgrade_levels_keyboard, missile_upgrade_settings_keyboard
)
...
def _stats_text(session, role: str, period: str) -> str:
    start, end = _period_range(period); users_all=get_users(session); admins=get_admins(session)
    countries=session.scalars(select(Country)).all()
    def ip(dt): return True if start is None else bool(dt and start<=dt<end)
    users=[u for u in users_all if ip(u.created_at)]; cutoff=end-timedelta(hours=24)
    active=[u for u in users if u.last_active_at and cutoff<=u.last_active_at<=end]; banned=[u for u in users if u.is_banned]
    admin_ids={a.telegram_id for a in admins}; au=[u for u in users if u.telegram_id in admin_ids]; aa=[u for u in au if u.last_active_at and cutoff<=u.last_active_at<=end]
    phone=sum(bool(u.phone_number) for u in users); uname=sum(bool(u.username) for u in users); country_users=sum(bool(get_user_countries(session,u)) for u in users)
    if role=="users": return (f"👥 <b>آمار کاربران</b> — {_period_label(period)}\n\n👥 کل کاربران: <b>{len(users):,}</b>\n🟢 فعال: <b>{len(active):,}</b>\n🔴 غیرفعال: <b>{len(users)-len(active):,}</b>\n🚫 بن‌شده: <b>{len(banned):,}</b>\n📱 دارای شماره: <b>{phone:,}</b>\n🔤 دارای نام کاربری: <b>{uname:,}</b>\n🌍 دارای کشور: <b>{country_users:,}</b>")
    if role=="admins": return (f"👨‍💼 <b>آمار ادمین‌ها</b> — {_period_label(period)}\n\n👨‍💼 کل ادمین‌ها: <b>{len(au):,}</b>\n🟢 فعال: <b>{len(aa):,}</b>\n🔴 غیرفعال: <b>{len(au)-len(aa):,}</b>\n📱 دارای شماره: <b>{sum(bool(u.phone_number) for u in au):,}</b>\n🔤 دارای نام کاربری: <b>{sum(bool(u.username) for u in au):,}</b>\n🌍 افراد دارای کشور: <b>{sum(bool(get_user_countries(session,u)) for u in au):,}</b>")
    return (f"📊 <b>آمار همه</b> — {_period_label(period)}\n\n👥 کاربران: <b>{len(users):,}</b>\n👨‍💼 ادمین‌ها: <b>{len(au):,}</b>\n🟢 فعال: <b>{len(active):,}</b>\n🚫 بن‌شده: <b>{len(banned):,}</b>\n📱 دارای شماره: <b>{phone:,}</b>\n🔤 دارای نام کاربری: <b>{uname:,}</b>\n🌍 افراد دارای کشور: <b>{sum(bool(get_user_countries(session,u)) for u in users):,}</b>")

def _period_label(period: str) -> str:
    return {"all":"آمار کلی", "today":"امروز", "yesterday":"دیروز", "week":"این هفته", "month":"این ماه", "year":"این سال"}.get(period, "آمار کلی")


from services.social_rewards import is_optional_claimed, is_optional_pending, get_optional_pending_entry, optional_timer_markup, optional_timer_text, optional_reward_delay_minutes, optional_claimed_count, scan_existing_optional_memberships, reset_social_item_state
...
async def resume_pending_broadcasts(app):
    ...
    while True:
        ...
        try:
            ...
            state=None
        if state and state.get("recipients"):
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if data.startswith("social:mandatory_check:"):
            ...
            from services.social_rewards import _mark_verified, _verified_count
            for item in mandatory:
        ...
        if data.startswith("missile_launch_cancel:") or data.startswith("missile_launch_confirm:"):
            ...
            try:
                ...
                if get_missile_count(inv, mid, level) <= 0:
                    # درخواست را نگه می‌داریم تا تأیید دوباره همان پیام هم
                    # وضعیت موجودی را نشان دهد و «لغو» همچنان معتبر باشد.
        ...
        if data.startswith("admin:"):
            ...
            if action == "message_edit":
                ...
                await query.answer(); await query.edit_message_text("📝 متن جدید پیام را ارسال کنید:",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="admin:messages")]])); return
            ...
            if action == "message_public":
                ...
                context.user_data[_waiting_scope_key("admin_message_pending")] = int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text("📢 <b>پیام عمومی</b>\n\nپیام مورد نظر را ارسال کنید:",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="admin:messages")]])); return
            if action == "message_public_target" and len(parts)>=3:
            ...
            if action == "user_country_delete_confirm" and len(parts) >= 4:
                ...
                await query.answer("🗑️ کشور حذف شد.", show_alert=True); await query.edit_message_text("⚙️ <b>تنظیمات بازی کاربر</b>", parse_mode="HTML", reply_markup=admin_user_game_settings_keyboard(target_id, context.user_data.get("user_game_settings_back", f"admin:user:{target_id}"))); return
            ...
            if action == "user_country_delete_cancel":
                ...
                await query.edit_message_text("⚙️ <b>تنظیمات بازی کاربر</b>", parse_mode="HTML", reply_markup=admin_user_game_settings_keyboard(target_id, back_callback))
                return
            ...
            if action == "user_game_settings" and len(parts) >= 3:
                ...
                await query.answer(); await query.edit_message_text("⚙️ <b>تنظیمات بازی کاربر</b>",parse_mode="HTML",reply_markup=admin_user_game_settings_keyboard(target_id,back_callback)); return
        ...
        if data in {"country_name_confirm", "country_name_change", "country_name_cancel", "country_name_edit_back"}:
            ...
            if data == "country_name_edit_back":
                pending_name = context.user_data.get(f"country_name_value_{query.from_user.id}")
                if not pending_name:
                    # اگر در حالت ویرایش نام جدید هستیم، بدون تغییر به مرحله دریافت نام برمی‌گردیم.
                    await query.answer("❌ نام قبلی وجود ندارد.", show_alert=True)
                    return
                await query.answer()
                await query.edit_message_text(f"🌍 نام کشور انتخابی: <b>{pending_name}</b>\n\nآیا تأیید می‌کنید؟", parse_mode="HTML", reply_markup=country_name_confirm_keyboard())
                context.user_data[f"country_name_pending_{query.from_user.id}"] = False
                return
        ...
        if data.startswith("shield:"):
            ...
            if shield_action == "list" and len(shield_parts) >= 4:
                ...
                from keyboards.main import shield_purchase_list_keyboard
                if not items:
        ...
        if data.startswith("owner:"):
            ...
            if action == "enigma_bank_type":
                typ=parts[2]; page=int(parts[3]) if len(parts)>3 else 1; from enigma.challenges import ensure_missions; data=ensure_missions(session); items=[m for m in data if m.get('type')==typ]
                if not items:
            ...
            if action == "enigma_delete_confirm":
                typ,mid=parts[2],int(parts[3]); from enigma.challenges import ensure_missions; data=ensure_missions(session); before=len(data); data[:]=[m for m in data if not (m['type']==typ and int(m['id'])==mid)]; enigma_set(session,'missions',data); await query.answer("🗑 مأموریت حذف شد.",show_alert=True); await query.edit_message_text("📚 <b>بانک انیگما</b>",parse_mode='HTML',reply_markup=owner_bank_keyboard(__import__('enigma.challenges',fromlist=['mission_bank_counts']).mission_bank_counts(session))); return
            ...
            if action == "enigma_mission_toggle":
                typ,mid=parts[2],int(parts[3]); from enigma.challenges import ensure_missions; data=ensure_missions(session); m=next((x for x in data if x['type']==typ and int(x['id'])==mid),None)
            ...
            if action == "social_item" and len(parts) >= 3:
                ...
                if item.get('type')=='optional_ad':
                    ...
                    from services.social_rewards import _verified_count
            ...
            if action == "general_settings":
                await query.answer()
                await query.edit_message_text("⚙️ <b>تنظیمات عمومی ربات</b>\n\nیکی از موارد زیر را انتخاب کنید:", parse_mode="HTML", reply_markup=general_settings_keyboard(get_bot_shutdown_mode(session)))
                return
            ...
            if action == "bot_shutdown":
                ...
                labels={"none":"👤 روشن برای کاربران عادی | 🛡️ روشن برای ادمین‌ها","users":"👤 خاموش برای کاربران عادی | 🛡️ روشن برای ادمین‌ها","admins":"👤 روشن برای کاربران عادی | 🛡️ خاموش برای ادمین‌ها","all":"👤 خاموش برای کاربران عادی | 🛡️ خاموش برای ادمین‌ها"}
                await query.edit_message_text(f"⏻ <b>کنترل دسترسی ربات</b>\n\nوضعیت فعلی: <b>{labels[mode]}</b>",parse_mode="HTML",reply_markup=bot_shutdown_keyboard(mode, back_callback="owner:general_settings")); return
            ...
            if action == "economy_arsenal":
                ...
                text = (
                    ...
                    f"📊 تنظیم سطح: <b>{len(cfg.get('levels',{}))}</b> سطح"
                )
            ...
            if action == "arsenal_build":
                ...
                text=("🏗 <b>ساخت زرادخانه</b>\n\n"
                      f"🏛️ سطح مرکز فرماندهی موردنیاز: <b>{int(cfg.get('build_required_hq_level',1))}</b>\n"
            ...
            if action == "arsenal_build_edit":
                ...
                context.user_data[_waiting_scope_key("economy_pending")] = int(update.effective_chat.id)
                label={"money":"💰 پول","metal":"🔩 فلز","fuel":"⛽ سوخت","uranium":"☢️ اورانیوم","storage_capacity":"📦 ظرفیت مخزن","military_power":"🪖 قدرت نظامی","strength":"🛡️ استحکام","required_hq_level":"🏛️ سطح مرکز فرماندهی موردنیاز","completion_time":"⏱️ زمان تکمیل"}[field]
                await query.answer(); await query.edit_message_text(
            ...
            if action == "arsenal_level":
                ...
                text=(f"🏭 <b>تنظیمات زرادخانه — سطح {level}</b>\n\n"
                      f"🏛️ سطح مرکز فرماندهی موردنیاز: <b>{data.get('required_hq_level',level)}</b>\n"
...
[InlineKeyboardButton("🔙 برگشت",callback_data="owner:economy_hq")]]
            ...
            if action == "exchange_text_edit":
                ...
                context.user_data[_waiting_scope_key("exchange_text_pending")] = int(update.effective_chat.id)
                await query.answer(); await query.edit_message_text("✏️ متن کامل جدید را ارسال کنید.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="owner:exchange_settings")]])); return
            if action == "exchange_text_cancel":
                ...
                await query.edit_message_text("💱 <b>تنظیم نرخ تمام تبادل‌ها</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت",callback_data="owner:exchange_settings")]])); return
            ...
            if action == "economy_metal_mine":
                ...
                text = (
                    ...
                    f"🔝 حداکثر سطح: <b>{cfg.get('max_level',100)}</b>\n"
                    f"☢️ اورانیوم تکمیل فوری: <b>{float(cfg.get('instant_finish_uranium_per_hour',0) or 0):,.2f}</b> در ساعت\n"
                    f"📊 تنظیم سطح: <b>{len(levels)}</b> سطح"
                )
                await query.answer()
            ...
            if action == "mine_build_edit":
                ...
                context.user_data[_waiting_scope_key("economy_pending")] = int(update.effective_chat.id)
                names = {"money":"💰 پول", "metal":"🔩 فلز", "fuel":"⛽ سوخت", "uranium":"☢️ اورانیوم", "military_power":"🪖 قدرت نظامی", "strength":"🛡️ استحکام", "production_per_hour":"⚙️ تولید در ساعت", "storage_capacity":"📦 ظرفیت مخزن", "completion_time":"⏱️ زمان تکمیل", "required_hq_level":"🏛️ سطح مرکز فرماندهی موردنیاز", "completion_time":"⏱️ زمان تکمیل"}
                await query.answer()
            ...
            if action == "mine_level":
                ...
                text = (f"⛏️ <b>تنظیمات معدن — سطح {level}</b>\n\n"
                        f"🏛️ سطح مرکز فرماندهی موردنیاز: <b>{int(data.get('required_hq_level', level))}</b>\n"
                        f"🪖 قدرت نظامی: <b>{data.get('military_power',0):,.0f}</b>\n🛡️ استحکام: <b>{data.get('strength',0):,.0f}</b>\n"
            ...
            if action == "mine_edit":
                ...
                context.user_data[_waiting_scope_key("economy_pending")] = int(update.effective_chat.id)
                names={"production_per_hour":"⚙️ تولید در ساعت","storage_capacity":"📦 ظرفیت مخزن","money":"💰 هزینه پول","metal":"🔩 هزینه فلز","fuel":"⛽ هزینه سوخت","uranium":"☢️ هزینه اورانیوم","military_power":"🪖 قدرت نظامی", "strength":"🛡️ استحکام", "required_hq_level":"🏛️ سطح مرکز فرماندهی موردنیاز", "completion_time":"⏱️ زمان تکمیل"}
                await query.answer(); await query.edit_message_text(
        ...
        if data == "arsenal_refresh":
            ...
            next_data=cfg.get("levels",{}).get(level+1,{}) if level>0 else {}
            ...
            if level > _before_level:
                ...
                await context.bot.send_message(arsenal_info(country,cfg), chat_id=int(query.message.chat_id), parse_mode="HTML", reply_markup=arsenal_keyboard(False,can_upgrade_arsenal(country,cfg)[0],False,float(cfg.get("levels",{}).get(level+1,{}).get("uranium",0))>0,construction_text=construction_label(country,"arsenal"),finish_uranium_cost=arsenal_instant_finish_uranium_cost(country,cfg)))
                return
            next_data=cfg.get("levels",{}).get(level+1,{}) if level>0 else {}
            can_up=can_upgrade_arsenal(country,cfg)[0]
            can_build_u=float(cfg.get("build_cost",{}).get("uranium",0))>0
            can_up_u=float(next_data.get("uranium",0))>0
            status=construction_label(country,"arsenal")
            await query.answer("🔄 زرادخانه به‌روزرسانی شد.")
            await query.edit_message_text(arsenal_info(country,cfg),parse_mode="HTML",reply_markup=arsenal_keyboard(level==0,can_up,can_build_u,can_up_u,construction_text=status,finish_uranium_cost=arsenal_instant_finish_uranium_cost(country,cfg)))
            return
        ...
        if data.startswith("hq_tech_upgrade:"):
            ...
            session.commit()
            if res=="DONE": await query.answer("✅ ارتقا کامل شد.",show_alert=True)
            else: await query.answer("⏳ ارتقا شروع شد.",show_alert=True)
            await query.edit_message_text(f"🧬 <b>فناوری نظامی — {escape(str(m.get('name','موشک')))}</b>\n\n⏳ <b>در حال ارتقا به سطح {target_level}</b>\n💳 روش پرداخت: <b>{'اورانیوم' if payment=='uranium' else 'پول، فلز و سوخت'}</b>\n⏱️ زمان ارتقا: <b>{float(data.get('upgrade_time',0) or 0):,.2f} ساعت</b>",parse_mode="HTML",reply_markup=missile_upgrade_levels_keyboard(mid,current_level,None, res=="IN_PROGRESS")); return
        ...
        if data.startswith("hq_tech_finish_now:"):
            ...
            if next_level:
                nd=levels.get(str(next_level),{}) or {}
                text += (f"\n\n⬆️ <b>سطح بعدی: {next_level}</b>\n"
                         f"🏛️ مرکز فرماندهی موردنیاز: <b>{int(nd.get('hq_required',m.get('hq_required',1)) or 1)}</b>\n"
                         f"🏭 زرادخانه موردنیاز: <b>{int(nd.get('arsenal_required',m.get('arsenal_required',1)) or 1)}</b>\n"
                         f"⏳ زمان ارتقا: <b>{float(nd.get('upgrade_time',0) or 0):,.0f} ساعت</b>\n"
                         f"🪖 قدرت: <b>{float(nd.get('power',0) or 0):,.0f}</b>\n"
                         f"⏱️ زمان برخورد با هدف: <b>{float(nd.get('target_time',0) or 0):,.0f} ثانیه</b>\n\n"
                         "💰 <b>هزینه‌ها:</b>\n"
                         f"💰 پول: <b>{float(nd.get('upgrade_money',0) or 0):,.0f}</b>\n"
                         f"🔩 فلز: <b>{float(nd.get('upgrade_metal',0) or 0):,.0f}</b>\n"
                         f"⛽ سوخت: <b>{float(nd.get('upgrade_fuel',0) or 0):,.0f}</b>\n"
                         f"☢️ اورانیوم: <b>{float(nd.get('upgrade_uranium',0) or 0):,.2f}</b>")
            await query.edit_message_text(text,parse_mode="HTML",reply_markup=missile_upgrade_levels_keyboard(mid,lvl,next_level)); return
        ...
        if data.startswith("hq_tech_refresh:"):
            ...
            await query.answer("🔄 به‌روزرسانی شد." if done else "⏳ هنوز زمان ارتقا تمام نشده است.",show_alert=not done)
            # بازخوانی همان صفحه
        ...
        if data == "hq_refresh":
            ...
            if int(getattr(country,"command_center_level",0)) > _before_level:
                ...
                await context.bot.send_message(hq_info(country,cfg), chat_id=int(query.message.chat_id), parse_mode="HTML", reply_markup=hq_keyboard(can_upgrade_hq(country,cfg)[0],built=int(country.command_center_level)>0,construction_text=construction_label(country,"command_center"),in_progress=bool(construction_label(country,"command_center")),finish_uranium_cost=hq_instant_finish_uranium_cost(country,cfg),build_uranium=float(hq_build_cost(cfg).get("uranium",0))>0,upgrade_uranium=float((hq_upgrade_cost(country.command_center_level,cfg) or {}).get("uranium",0))>0))
                return
            cfg = get_hq_config(session)
            status = construction_label(country, "command_center")
            await query.answer("🔄 به‌روزرسانی شد.")
