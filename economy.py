...
def purchase(session,user_id,item_id,country):
    ...
    cooldown=purchase_cooldown_remaining(session,user_id,item_id,country_id)
    if cooldown>0:
    ...
    for skey, rec in s.items():
        if int(rec.get('country_id',0) or 0)!=country_id: continue
    ...
    if same_type:
        ...
        key=_shield_key(user_id,item_id,country_id)
        old=s.get(key) or {}
        old_exp=float(old.get('expires_at',0) or 0)
        expires=max(now,old_exp)+duration
        s[key]={'user_id':int(user_id),'country_id':country_id,'item_id':str(item_id),'type':item.get('type'),'expires_at':expires,'purchased_at':now}
...
def adjust_active_shield_time(session, user_id, shield_type, delta_hours, country_id=None):
    """زمان سپر فعال کاربر را به‌صورت ساعتی افزایش/کاهش می‌دهد."""
    if shield_type not in {'global','continental'}:
        return False, 0, None
    ...
    for key,v in s.items():
        # همیشه کاربر را هم محدود کن؛ وقتی country_id داده شده، صرفاً کشور کافی نیست.
        if int(v.get('user_id',0) or 0)!=int(user_id): continue
        if country_id is not None and int(v.get('country_id',0) or 0)!=int(country_id): continue
        expires=float(v.get('expires_at',0) or 0)
        if expires<=now: continue
        item=cfg.get(str(v.get('item_id')))
        typ=(item.get('type') if item else v.get('type'))
        if typ==shield_type:
            candidates.append((key,v,expires,item))
    if not candidates:
        if delta < 0:
            return False, 0, None
        # افزایش بدون سپر فعال: یک سپر مصنوعی قابل‌مشاهده ایجاد می‌کنیم.
        key=_shield_key(user_id,f'__admin_{shield_type}__',country_id) if country_id is not None else f'active:{int(user_id)}:__admin_{shield_type}__'
        expires=now+delta*3600
        s[key]={'user_id':int(user_id),'country_id':int(country_id) if country_id is not None else None,'item_id':f'__admin_{shield_type}__','type':shield_type,'name':('سپر جهانی' if shield_type=='global' else 'سپر قاره‌ای'),'expires_at':expires,'purchased_at':now,'admin_granted':True}
        _save_state(session,s); session.commit()
        return True, delta*3600, expires
    key,v,old,item=max(candidates,key=lambda x:x[2])
    new=max(now, old + delta*3600)
    if new<=now:
        s.pop(key,None); actual=old-now
        expires=None
    else:
        v['expires_at']=new; actual=(new-old)
        expires=new
    _save_state(session,s); session.commit()
    return True, actual, expires
...
def active_for_target(session,user_id,attacker_continent=None,target_continent=None,country_id=None):
    ...
    for k,v in s.items():
        ...
        elif int(v.get('user_id',0) or 0)!=int(user_id): continue
        if float(v.get('expires_at',0) or 0)<=now: continue
        item=cfg.get(str(v.get('item_id')))
        typ=(item.get('type') if item else v.get('type'))
        if not typ: continue
        if typ=='global' or (attacker_continent is not None and target_continent is not None and str(attacker_continent)==str(target_continent)):
            vv=dict(v); vv['name']=(item.get('name') if item else v.get('name','سپر خودکار')); vv['type']=typ; out.append(vv)
...
def register_group_attack_received(session,target_user_id,group_id,country_id=None):
    ...
    s=cleanup(session); now=time.time(); skey=_shield_key(target_user_id,'__auto_global__',country_id) if country_id is not None else f'active:{int(target_user_id)}:__auto_global__'
    old=s.get(skey) or {}; expires=max(now,float(old.get('expires_at',0) or 0))+settings['auto_shield_hours']*3600
    s[skey]={'user_id':int(target_user_id),'country_id':int(country_id) if country_id is not None else None,'item_id':'__auto_global__','type':'global','name':'سپر خودکار','expires_at':expires,'purchased_at':now,'automatic':True}
