...
def installment_settings_keyboard(loan_interval_hours=24, reminder_time="09:00", lock_days=1, overdue_multiplier=2):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"🔒 مدت بسته بودن بانک پس از عدم پرداخت: {float(lock_days):g} روز", callback_data="owner:bank_loan_lock_days_edit", style="primary")],
...
def bank_level_settings_keyboard(level, build=False):
    level = int(level)
    back_cb = "owner:bank_build" if build or level <= 1 else f"owner:bank_level:{level}"
    return InlineKeyboardMarkup([
...
def bank_investments_settings_keyboard(plans, level=1, build=False):
    ...
    rows.append([InlineKeyboardButton("🔙 برگشت", callback_data=("owner:bank_build" if mode else f"owner:bank_level:{level}"))])
    return InlineKeyboardMarkup(rows)
...
def bank_investment_list_keyboard(plans, level=1, build=False, delete_mode=False):
    ...
    for i, plan in enumerate(plans):
        ...
        rows.append([InlineKeyboardButton(f"{icon} {name}", callback_data=f"owner:bank_investment_{action}:{level}:{i}:{mode}")])
...
def bank_investment_edit_keyboard(index, level=1, build=False):
    level = int(level)
    mode = 1 if build or level <= 1 else 0
    back_cb = f"owner:bank_investments:{level}:{mode}"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ نام", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:name", style="primary")],
        [InlineKeyboardButton("🧩 نوع", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:plan_type", style="primary")],
        [InlineKeyboardButton("💰 حداقل مبلغ", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:min_amount", style="primary")],
        [InlineKeyboardButton("💰 حداکثر مبلغ", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:max_amount", style="primary")],
        [InlineKeyboardButton("🎚️ ریسک", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:risk", style="primary")],
        [InlineKeyboardButton("📈 سود احتمالی (%)", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:profit_percent", style="primary")],
        [InlineKeyboardButton("📉 زیان احتمالی (%)", callback_data=f"owner:bank_investment_edit:{level}:{index}:{mode}:loss_percent", style="primary")],
