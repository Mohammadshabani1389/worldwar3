...
def installment_settings_keyboard(loan_interval_hours=24, reminder_time="09:00"):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"🔔 زمان یادآوری قسط: {reminder_time}", callback_data="owner:bank_loan_reminder_time_edit", style="primary")],
...
def bank_economy_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💳 تنظیمات وام", callback_data="owner:bank_loans", style="primary")],
        [InlineKeyboardButton("📈 تنظیمات سرمایه‌گذاری", callback_data="owner:bank_investments", style="primary")],
...
def bank_build_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏦 تنظیمات سپرده", callback_data="owner:bank_build_deposit_settings", style="primary")],
        [InlineKeyboardButton("💳 بخش تنظیمات وام", callback_data="owner:bank_build_loan_settings", style="primary")],
        [InlineKeyboardButton("📈 تنظیمات سرمایه‌گذاری", callback_data="owner:bank_investments", style="primary")],
        [InlineKeyboardButton("🧾 تنظیمات مالیات", callback_data="owner:bank_tax_settings:1", style="primary")],
...
def bank_level_edit_keyboard(level):
    ...
    if level <= 1:
        fields = [
            ...
            ("🏦 تنظیمات سپرده", "owner:bank_deposit_settings:1"),
            ("🧾 تنظیمات مالیات", "owner:bank_tax_settings:1"),
        ]
    else:
        fields = [
            ("🏛️ سطح مرکز فرماندهی مورد نیاز", "required_hq_level"),
            ("⏱️ زمان تکمیل", "upgrade_time_hours"),
            ("💰 هزینه‌ها", f"owner:bank_level_costs:{level}"),
            ("🏦 تنظیمات سپرده", f"owner:bank_deposit_settings:{level}"),
            ("💳 بخش تنظیمات وام", f"owner:bank_loan_settings:{level}"),
            ("🧾 تنظیمات مالیات", f"owner:bank_tax_settings:{level}"),
        ]
    rows = []
...
def bank_tax_settings_keyboard(level):
    level=int(level)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⬇️ حداقل مالیات هر شهروند", callback_data=f"owner:bank_edit:{level}:tax_min", style="primary")],
        [InlineKeyboardButton("⬆️ حداکثر مالیات هر شهروند", callback_data=f"owner:bank_edit:{level}:tax_max", style="primary")],
...
def bank_loans_settings_keyboard(level=1, build=False, loan=None):
    ...
    back_cb = "owner:bank_build" if build else f"owner:bank_level:{level}"
    ...
    fields = [
        (f"💰 مبلغ: {min_amount:,.0f} تا {max_amount:,.0f}", "amount_range"),
    ...
    rows=[]
    for label, field in fields:
        cb = prefix + field if field in {"amount_range","interest_range","installments_range"} else prefix + field
...
def bank_investments_settings_keyboard(plans):
    rows=[[InlineKeyboardButton(f"📈 {p.get('name','سرمایه‌گذاری')}", callback_data=f"owner:bank_investment:{i}", style="primary")] for i,p in enumerate(plans)]
    rows.append([InlineKeyboardButton("➕ افزودن سرمایه‌گذاری", callback_data="owner:bank_investment_add", style="success")])
    rows.append([InlineKeyboardButton("🔙 برگشت", callback_data="owner:economy_bank")])
    return InlineKeyboardMarkup(rows)

def bank_investment_edit_keyboard(index):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ نام", callback_data=f"owner:bank_investment_edit:{index}:name", style="primary")],
        [InlineKeyboardButton("🧩 نوع", callback_data=f"owner:bank_investment_edit:{index}:plan_type", style="primary")],
        [InlineKeyboardButton("💰 حداقل مبلغ", callback_data=f"owner:bank_investment_edit:{index}:min_amount", style="primary")],
        [InlineKeyboardButton("💰 حداکثر مبلغ", callback_data=f"owner:bank_investment_edit:{index}:max_amount", style="primary")],
        [InlineKeyboardButton("🎚️ درجه سختی", callback_data=f"owner:bank_investment_edit:{index}:difficulty", style="primary")],
        [InlineKeyboardButton("📈 سود احتمالی (%)", callback_data=f"owner:bank_investment_edit:{index}:profit_percent", style="primary")],
        [InlineKeyboardButton("📉 زیان احتمالی (%)", callback_data=f"owner:bank_investment_edit:{index}:loss_percent", style="primary")],
        [InlineKeyboardButton("⏱️ مدت (ساعت)", callback_data=f"owner:bank_investment_edit:{index}:duration_hours", style="primary")],
        [InlineKeyboardButton("🔄 فعال/غیرفعال", callback_data=f"owner:bank_investment_toggle:{index}", style="primary")],
        [InlineKeyboardButton("🗑️ حذف طرح", callback_data=f"owner:bank_investment_delete:{index}", style="danger")],
        [InlineKeyboardButton("🔙 برگشت", callback_data="owner:bank_investments")],
    ])
