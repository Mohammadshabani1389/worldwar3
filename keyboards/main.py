...
def command_keyboard(no_country: bool = False):
    return _reply_page_keyboard(
        [
            ...
            [KeyboardButton("🏭 زرادخانه"), KeyboardButton("🏦 بانک مرکزی"), KeyboardButton("⛏️ معدن فلز")],
            [
...
def central_bank_keyboard(built=True, can_upgrade=False, in_progress=False, finish_uranium_cost=0.0, session=None, country=None):
    ...
    if in_progress:
        ...
        rows.append([InlineKeyboardButton("❌ لغو ساخت/ارتقا", callback_data="bank:cancel")])
    else:
        if can_upgrade:
            rows.append([InlineKeyboardButton("⬆️ ارتقای بانک مرکزی", callback_data="bank:upgrade")])

        # مشخصات هر بخش مستقیماً داخل همان دکمه نمایش داده می‌شود تا
        # صفحه اصلی بانک فقط نقش منوی انتخاب بخش‌ها را داشته باشد.
        deposit_label = "💰 سپرده"
        loan_label = "💳 وام"
        investment_label = "📈 سرمایه‌گذاری"
        population_label = "👥 جمعیت و مالیات"
        if session is not None and country is not None:
            try:
                from services.economy import bank_level_config, get_bank_config, accrue_bank_profit, ensure_bank_account
                ...
                tax_min=float(d.get("tax_min", 0) or 0)
                tax_max=float(d.get("tax_max", 0) or 0)
                deposit_label=(f"💰 سپرده | ظرفیت: {float(d.get('deposit_capacity',0) or 0):,.0f} | "
                               f"موجودی: {float(acc.deposit_principal or 0):,.0f} | "
                               f"سود: {float(d.get('deposit_hourly_profit_percent',0) or 0):g}%/ساعت")
                loan_label=(f"💳 وام | مبلغ: {float(loan.get('min_amount',0) or 0):,.0f} تا "
                            f"{float(loan.get('max_amount',0) or 0):,.0f} | "
                            f"سود: {float(loan.get('min_interest_percent',0) or 0):g}% تا "
                            f"{float(loan.get('max_interest_percent',0) or 0):g}%")
                investment_label=f"📈 سرمایه‌گذاری | طرح‌های فعال: {len(plans)}"
                population_label=(f"👥 جمعیت و مالیات | جمعیت: {int(acc.citizens or 0):,} | "
                                  f"مالیات: {float(acc.tax_per_citizen or 0):,.0f} | "
                                  f"محدوده: {tax_min:,.0f} تا {tax_max:,.0f}")
            except Exception:
                pass
        rows += [
            [InlineKeyboardButton(deposit_label, callback_data="bank:deposit")],
            [InlineKeyboardButton(loan_label, callback_data="bank:loans")],
            [InlineKeyboardButton(investment_label, callback_data="bank:investments")],
            [InlineKeyboardButton(population_label, callback_data="bank:population")],
        ]
    return InlineKeyboardMarkup(rows)

def central_bank_deposit_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💰 سپرده‌گذاری", callback_data="bank:deposit_input", style="success")],
        [InlineKeyboardButton("💵 برداشت سود", callback_data="bank:withdraw_profit")],
        [InlineKeyboardButton("💵 برداشت کل سپرده", callback_data="bank:withdraw_all")],
...
def central_bank_withdraw_confirm_keyboard(kind):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("❌ لغو", callback_data="bank:deposit", style="danger")],
    ])
...
def central_bank_loan_quote_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("❌ لغو", callback_data="bank:loans", style="danger")],
    ])

def central_bank_loan_detail_keyboard(loan_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💵 پرداخت قسط بعدی", callback_data=f"bank:loan_pay:{int(loan_id)}", style="success")],
...
def central_bank_investment_confirm_keyboard(index):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("❌ لغو", callback_data=f"bank:investment_plan:{int(index)}", style="danger")],
    ])
